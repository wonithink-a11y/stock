...
3. **실전 TR_ID 를 공식 예제와 대조한다.** — 2026-09-22 대조 완료: TR_ID 13개 일치, 주문 본문 2건 수정
   (docs/control/autotrader-실전TRID-대조-2026-09-22.md). 남은 것은 새 본문을 모의 주문으로 확인하는 것뿐.
...
- 실전 TR_ID·해외 매도 `SLL_TYPE`·시세 TR 은 2026-09-22 공식 예제와 대조했다(§4-3). 조회 TR 이 틀리면 조회가 실패할 뿐이지만 **주문 TR 이 틀리면 거절되거나 다른 동작일 수 있다.**
