"""probe-bithumb-paper-pollonce-smoke.py — BithumbPaperBroker로 poll_once()
상태기계 전체를 실제 빗썸 공개 시세로 1회 왕복 검증한다. 키 불필요, 진짜
주문 없음(probe-upbit-paper-pollonce-smoke.py와 동일 구조).

PENDING_ENTRY를 이 스크립트가 직접 심는다(scan_signals 대신 - 배관 검증이
목적이지 신호 로직 검증이 아니다). OPEN 전이 후에는 stop_price를 현재가보다
훨씬 위로 올려 즉시 STOP 청산을 유도한다.

사용:
    python scripts/probe-bithumb-paper-pollonce-smoke.py
"""
import sys
import time
...
from engine.live.bithumbPaperBroker import BithumbPaperBroker  # noqa: E402

STRATEGY_ID = "bithumb_paper_pollonce_smoke"
MARKET = "KRW-BTC"
...
broker = BithumbPaperBroker()

print()
print("  poll_once() 빗썸 모의(로컬 시뮬레이션) 왕복 검증")
...
positionStore.save(str(REPO), STRATEGY_ID,
                    {MARKET: {"status": "PENDING_ENTRY", "quantity": 0.0001, "intent_date": "2026-09-14"}})
...
print("       BithumbPaperBroker로 poll_once() 상태기계 전체 검증 완료(실제 주문 없음).")
