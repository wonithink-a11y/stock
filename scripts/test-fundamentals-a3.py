...
def _with_states(states, fn):
    """_state-N.json을 임시 디렉터리에 깔고 fn(state_dir)을 부른다. 저장소를 안 건드린다."""
    import tempfile
...
b6, d6 = _with_states(_others, lambda d: m.shard_budget(_POL, 8, 6, 2005, _TODAY, d))
...
b0, d0 = _with_states(_all_active, lambda d: m.shard_budget(_POL, 8, 0, 0, _TODAY, d))
...
for s in range(8):
    ...
    bb, _ = _with_states(others, lambda d, s=s, mine=mine: m.shard_budget(
        _POL, 8, s, mine["callsUsedToday"], _TODAY, d))
    _tot += bb
...
b3, d3 = _with_states(_stale, lambda d: m.shard_budget(_POL, 8, 3, 0, _TODAY, d))
...
b_none, d_none = _with_states({}, lambda d: m.shard_budget(_POL, 8, 0, 0, _TODAY, d))
