...
Validated against
  ...
  다음      A5-3 게이트 3 — 범위 결정 대기(2026-08-12, WAITING_USER). shareholderReturn·
            peg는 종목 단위 계산이라 구현 가능하지만 그 둘만 열어도
            availableWeight=0.59로 게이트(0.6) 미달이다. perRelative(업종 중앙값
            대비 PER)까지 있어야 0.68로 넘는데, 이건 그 시점 업종 동료 전체의
            PER을 함께 보는 횡단면 계산이라 resolver.js의 종목 단위 인터페이스에
            안 맞고, 라이브 경로(collect.js)처럼 날짜별·업종별 PIT 중앙값 인프라를
            새로 만들어야 한다. 전체 구현/부분 구현(게이트 미해결)/보류 중 결정 필요
  언제든    LAB-2(FY2015 EPS 라벨링) 방향 보류 — 서두를 이유 없음(2026-08-12)
