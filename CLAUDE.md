...
Validated against
  ...
  안 한다   ★★ **O2b·O3u 모의 실행 — 중단**(2026-09-22, 설계 §5 1단계 시험 실패). 매수 PASS(종가 274,000, 0.0bp) ·
            **매도 FAIL**(체결 282,500 vs 공식 시가 283,000, −17.67bp — 분봉으로 교차 확인, 모의가 시초 단일가가 아니라 개장 뒤
            연속 매매가로 체결한 것으로 보임). 결과·해석: docs/control/단기규칙-모의실행-1단계시험-결과-2026-09-22.md.
            다른 체결 방식으로 계속하는 것은 **새 GO 없이 안 한다**. O2b **그림자**(반사실 계산)는 무관 — 월간 점검에서 계속.
  완료      2026-09-22 아침 VM 확인 — 미국 S&P500 스냅샷 첫 자동 실행 성공(07:31) · RV20·크립토 09:05 정상(git pull 수정 b61f5882
            첫 검증 통과, `systemctl --failed` 비어 있음) · 크립토 AVAX +3,558 · INJ −5,051 청산(~48시간 보유 편차), 신규 5종 진입.
            RV20 30분 지연은 09-14 부터 매일이었다 — 수집기가 휴장일을 매번 재조회(60초 대기). 09-23 수정 5e3eddfa, VM 재실행 1.5초.
            **09-23 09:05 실물 검증 통과 — 15초 종료**(percentile 0.353→1.0x, 보유 1계약 = 목표, 주문 없음). 장중 알림 `[object Object]` 해소 확인.
  다음      ★★ 연구 라인 전부 종결(세션인수인계-2026-09-21-b.md).
            ...
            남은 길은 forward 그림자(별도 사전등록 c42e17dc). ★ **업종 ETF 모멘텀(09-22) INCONCLUSIVE** — 적격 단위 15개 이상인 달이
            199개월 중 82개월, TRAIN 15개월(기준 48)이라 판정 불가. 권고: 라인 종결(forward 는 월 1관측이라 비용 대비 가치 낮음). 새 연구 착수 전 새 데이터·새 표본 없이는 회계·가치·품질 축 재시도 금지(각 사전등록 §6).
  다음      ★★ autotrader — 2026-09-22 프로필(키·전략·한도·예약)·무한매수 플러그인(옛 러너와 VM 8/8 일치)·웹 조작·
            ...
            nginx 정본은 deploy/nginx-*.conf. 세부: docs/control/세션인수인계-2026-09-26-c.md(최신 — 해독 요약·목표가·업종 재분류 21종목·미국 PIT 게이트·us-map-1.0·미국 PBR REJECT·KIS 토큰 사고) · -2026-09-26-b.md(워뇨띠 후속) · -2026-09-26.md(알림 VM 이전·미국 8-K·us-pit 수집) · -2026-09-24.md(09-28 국내 모의 1주 시험 예약) · -2026-09-23.md · -2026-09-22-b.md · -2026-09-22.md ·
  ...
  안 한다   ★ 워뇨띠 BitMEX 매매기록 복제(2026-09-22 탐색) — **REJECT·종결**. 재현 가능한 신호 없음, 캠페인 성패는 초기 행동으로
            예측 불가. ★ '체결 뒤 +8bp' 는 분 종가 착시(체결가 기준 ~0) — 이 숫자로 역추세 단기 규칙을 다시 연구하지 않는다.
            재론 조건: 주문 접수·취소 기록이나 틱·호가 데이터. findings/wonyotti-bitmex-replication-exploration-2026-09.md.
            ★ 09-26 후속 셋 다 **REJECT**(사전등록): 운용 구조(급락 분할 매수+되돌림 청산, 432조합 — BTC OOS 무작위보다 나쁨) ·
            어림수 지지·저항(가짜 격자가 더 큼) · 무한매수법 크립토 18변형(같은 비중 보유보다 수익 절반, FTT 붕괴 손실).
            펀딩 극단 역행은 C6·Step14 로 이미 기각 — 재시험 안 함. BitMEX 공개 틱·호가(public.bitmex.com)는 존재 확인, 기대값 낮아 미수집.
  안 한다   ★ Stine 조용한 재진입(2026-09-23, 사전등록 30b5b240) — **REJECT·종결**. 돌파 추격(A)·2~6주 뒤 눌림(B) 둘 다 무작위 대조보다 나쁨
            (B TRAIN t 0.19 < 바닥선 2.26, OOS gross 음 — 비용 아닌 신호 문제). 실적·내부자 필터를 그 위에 쌓지 않는다. 추격 OOS 큰 음수를
            공매도로 뒤집는 것도 사후 가설 — 안 함. ★ 가격 A2a·A2b 는 수정주가인데 **거래량은 원값**(분할 시 가짜 급증). findings/stine-quiet-entry-results-2026-09.md.
  안 한다   ★ 스캘핑 후보 6개(외부 제안, 2026-09-27) — **새로 잰 3갈래 전부 REJECT**, 나머지 3개는 기존 기록으로 닫힘(사전등록 70d64249·852180bd).
  ...
  안 한다   ★ 미국 S&P500 저PBR 재현(2026-09-26, 사전등록 a881dc18 · 구현 40930672) — **REJECT**. B/M 상위 10% 월 +43.6bp(t 1.44 < 2.0),
            ...
            findings/us-pbr-revalidation-results-2026-09.md.
  안 한다   ★ 국내 자기 과거 대비 PBR 밴드(2026-09-26, 사전등록 ae2ab2b9 · 새 표본 2010~2015 KRX PBR 로그인 수집) — **REJECT**. 밴드 하위 20%
            월 −69bp(t −2.34, 6개 연도 전부 음수)·월봉 양봉 추가도 같음·횡단면 저PBR 대비도 음(증분 없음). 좁힐수록 나쁨(가치 함정).
            반대 방향(모멘텀류)은 사후 가설 — 이 표본으로 채택 안 함. findings/kr-own-pbr-band-results-2026-09.md.
  안 한다   ★ 테마 선행관계 A형(2026-09-24, 사전등록 e959ed21 · 코드 ade8b040) — **REJECT·연결선 0개**. 소테마 43 양방향·h 6개,
            PIT 규모(A4 실제가×A3c 공시 주식수) 통제, 가족 P 156·S 10,332 발견 통과 0(최대|t| 2.40/4.43 < 바닥선 3.56/4.64).
            음성 대조로 규모 선후행은 실재 확인. 시차·쌍을 바꿔 재실행 금지. findings/theme-leadlag-results-2026-09.md.
            ★ **B형(2024~ 신규 구조, P 156 후보)은 동결 다음 거래일부터 500거래일(≈2028-10) 뒤 1회 판정** — 그 전 계산·중간 판정 금지.
            ★ 판정은 테마 트리 **v0.1**(`git show e959ed21:config/themeTree.json`)로 한다 — 현재 파일은 09-26 v0.2(가전 신설).
            테마 트리 v0.1(config/themeTree.json, 대시보드 섹터강도 탭, daily-analysis 단계)과 1단계 동조 지도(fa225d01)는 관찰용으로 유지.
