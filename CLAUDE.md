...
(2026-09-04 사용자 지시). 셋 다 참이어야 한다 — ① **일일 한도가 있는 외부
API(DART 4만콜·KIS)를 안 쓴다** — 저장소 데이터만 읽거나, 쓰더라도 한도 없는
무료 소스(KRX·네이버)에 몇 분짜리다 ② 결과가 `git revert`로 되돌아간다
③ 재실행이 멱등하다. 돌린 사실은 한 줄로 보고한다.
