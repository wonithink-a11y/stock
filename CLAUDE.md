...
Validated against
  ...
  다음      ★★ 2026-09-21 **진행 중 4건**(세션인수인계-2026-09-21.md). (1) **A3e 확장 재무 수집**(연구 전용, 사용자 승인) —
            `research/strategy-lab/build_extended_financials_panel.py` 가 로컬에서 도는 중(24,750 corp-year, 하루 예산 30,000콜,
            재개 가능: 끊기면 같은 명령을 다시 실행). 산출 `data/fundamentals-ext/`(gitignore). 완료 후 계정 매핑 감사 →
            EV/EBIT·FCF·품질 계열 사전등록(설계 docs/control/A3e-확장재무수집-설계-2026-09-21.md). **EV/EBITDA 는 감가상각비 커버리지
            27% 라 전면 불가.** (2) **모의 동시호가 1주 시험** — VM cron 일회성 예약(09-21 15:24 매수 · 09-22 08:45 매도, 대조 포함,
            끝나면 자동 제거, 로그 `~/collector-venv/logs/auction-probe.log`). 통과해야 O2b·O3u 모의 실행(설계 동결
            docs/control/단기규칙-모의실행-설계-2026-09-20.md, 슬리브당 4천만원)이 이어진다 — **실패하면 그 실험 중단**.
            (3) **미국 S&P500 스냅샷 타이머**(화~토 07:30, 첫 자동 실행 09-22 — 로그·`_status.json` 확인). 생존편향 경계는
            `_meta.json`. (4) ETF 횡단면(세금·분류 확인 완료, 국내 주식형 152/263)은 **데이터 수집 승인 대기**(시험 결과 뒤).
            **애널리스트 목표가·투자의견**은 KIS 오픈API `invest-opinion`(FHKST663300C0)으로 2010년부터 받을 수 있음을 확인(무료, 100행/호출).
            **다음 세션 착수: ① KIS 목표가 수집 설계 ② 유상증자 회피 필터(PBR 슬리브) 사전등록 ③ 애널리스트 추정치(EPS·영업이익 리비전) 소스 조사** — 상세는 세션인수인계-2026-09-21.md §6.
