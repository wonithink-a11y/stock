  ...
  완료      ★ 2026-09-04 상태블록 반영 누락분 다섯 건
            ① **criteria KR-2.4 승격**(🔴 GO, `31e72e1`) — axis-weight-sweep-2026-09
              ...
              **A5 재백필은 안 했다** — criteria 해시가 바뀌어 계약상 대상이지만
              그건 '실행'이라 별도 사전 확인이다.
            ② **PF-1.1 초안 — 여전히 미발효**(`d9549ed`, registry 미등록). PF-1.0이
...
외부 API 예산을 쓰는 수집 · VM 배포 · 상태 디렉터리 삭제 · 실주문 경로.
코드는 revert로 되돌아가지만 이틀짜리 수집과 API 예산은 그렇지 않다.
(push는 🟡에 한해 2026-08-17에 이미 완화됐다 — 커밋하면 그대로 push한다)

**예외 — 되돌릴 수 있고 예산을 안 쓰는 Actions 수동 트리거는 바로 돌린다**
(2026-09-04 사용자 지시). 셋 다 참이어야 한다 — ① **대량 수집이 아니다**
— DART 일일 4만콜 예산이나 수천 종목 스캔을 갉아먹지 않는다(저장소 데이터만
읽거나, 잔고조회 몇 건·한도 없는 무료 소스(KRX·네이버)에 몇 분짜리는 해당
없음) ② 결과가 `git revert`로 되돌아간다 ③ 재실행이 멱등하다.
돌린 사실은 한 줄로 보고한다.

```
바로 돌린다   deploy-pages · docs/data/ 재빌드류(ETF·섹터강도·findings·
              macro·market-flows·watchlist) · paper-trading-ui · 재배포
확인받는다    A2b·A3x·A4·분봉 수집(DART·KIS·KRX 예산) · exit-overlay(DART 호출)
              A5 채점 백필(예산은 0이지만 125만 행을 덮어쓴다 — ②③은 참이라
              경계다, 지금은 확인 쪽에 둔다)
```

판단이 갈리면 확인 쪽이다. 이 완화는 '무엇을 소모하는가'로 가르는 것이지
'급해 보이는가'로 가르는 것이 아니다.
