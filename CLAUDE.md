...
Validated against
  ...
  다음      ★ 무한매수법 해외 슬리브 — **월요일(09-14) 잔존 주문 확인이 선결**.
            2026-09-12 에 조사→백테스트→엔진→실주문까지 한 번에 갔다. 모의투자
            해외계좌(외화 예수금 $100,000, 원화와 **별도**)에 TQQQ 15주·SOXL 8주가
            실제로 들어가 있다(T 0.86/0.79 of 40분할 — 1회차도 못 채운 상태다).
            ★ **미국 지정가 주문이 당일물인지 모른다** — 남아 있으면 매일 돌릴 때
            같은 주문이 쌓인다. `open_orders()` 로 확인하고 남으면 취소 후 재접수
            로직을 넣는다. **이게 매일 자동 실행 배선의 선결 조건이다.**
            ...
            판정은 **안 냈다** — 표본이 2010-02 시작이라 닷컴·리먼이 없다.
