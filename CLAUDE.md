...
Validated against
  ...
  다음      ★ 2026-09-02 팩터 조합 실험 인프라 신설 — 사용자 요청("여러 조합을
            ...
            **검증한 축 54개 중 채택 0개 — 업종중립 PBR 계열도 포함해서 0개
            (2026-09-18 정정, 아래).** 유동성 조합·공매도(신규 데이터원 A8)·
            46축 빔 최고(t=6.08)가 전부 난수선은 넘고 **OOS에서 반전/붕괴**.
            업종중립 PBR 계열은 세 구간 부호 유지 + 비용 통과 + 엔진 통과로
            한때 HOLD였으나(09-02), 그 HOLD의 진짜 블로커였던 "제한된
            난수 바닥선"을 다음날(09-03) 전체 53축·top-30 선택규칙 기준으로
            직접 재서 **REJECT로 종결**됐다 — 후보가 1,431조합 중 76위(t=2.11,
            진짜 바닥선 3.52의 60% 수준), 바닥선을 넘은 11개는 전부 이미
            기각된 유동성·공매도 축이었다(sector-neutral-pbr-top30-full-
            universe-nullbar-2026-09.md, "라인을 종료한다"). 이 라인은
            **더 이상 다음 단계가 없다** — 재론 안 함.
  ...
  완료      상세 이력은 docs/control/완료-이력.md 참고(2026-09-06, CLAUDE.md가
            ...
            없음, 원본 그대로 이동). 최신 항목:
            ★ 2026-09-20 **차트 구조형 단기 35셀(Sweep·MSS·FVG·Spring·PDL·흡수·드라이브·재테스트)
            — ECONOMIC 0**(사전등록 a812cb1, findings/structure-patterns-results-2026-09.md).
            **실제 KR 왕복 비용을 재계산**했다 — KIS 수수료 무료로 줄어드는 건 약 3bp 뿐이고
            거래세 20bp(2026 코스피·코스닥 매도, 다수 출처 확인)+호가 스프레드(평균 12.6bp,
            저가주 많음)라 이 유니버스 평균 **33.5bp — 기존 30bp 관례보다 높다**. 유일한 방향성
            신호는 스윕→MSS→FVG 재진입 롱(gross +7~25bp, net −1~−26bp)이고 접촉=체결 가정이
            t 를 부풀렸다(13.9→엄격 체결 4.5). 크립토 흡수 4셀은 이벤트 2건이라 판정불가.
            재개 조건: 비용 낮은 수단(선물 표본 2배·거래세 인하).
            ★ 2026-09-20 **종가→익일 시가 14셀 — ECONOMIC 0, HOLD**(사전등록 9f8cecf, findings/close-open-results-2026-09.md).
            **비용 정정**: KIS 이벤트는 **코인원 WTS 주문 한정·9월 한시**(우리 API 주문엔 미적용), 매도 농특세는 유지 —
            지속 기준은 정상 수수료 포함. 세금 20bp 는 왕복 1회(이중 계상 아님). 종가·시가 **단일가** 체결이면 스프레드 0,
            왕복 23.54bp(세금 20+수수료 2.81+유관 0.73). 사실: 강한 종가·전일고가 돌파(O2b)·급등(O3u) → 밤사이 gross +19~34bp,
            익일 장중 −35~−117bp(갭상승 롱 −117bp). net 은 10년 OOS 손익분기(+2/−0.6bp, 스트레스 음)·체제 의존(2022 −15, 2026 +21).
            15:20 신호로 옮겨도 gross 동일(룩어헤드 아님).
            ★ **O2b forward 그림자 가동·동결**(2026-09-20 사용자 GO, 6d2fd27) — docs/control/O2b-그림자-설계-2026-09-20.md,
            `research/strategy-lab/futures/run_close_open_shadow.py`. 반사실(주문 없음), 신호일 ≥ 2026-08-22, **250일에 1회 판정**
            (net_durable 평균>0 · t≥2 · 스트레스>0 = KEEP 후보, 평균≤0 = REJECT, 그 외 500일까지 1회 연장). t≥2 는 net ≈ +18bp 이상이어야 넘는다
            (일별 sd ≈139bp) — 10년 OOS 수준(+2)이면 INCONCLUSIVE 가 기대치. **첫 관측 17일: gross −38bp(시장 −16, 초과 −22, t −1.5) — 너무 이르다**
            (신호일 시장 밤사이가 급락 다발). 로그 `reports/2026-09-o2b-shadow/observations.jsonl`(추적, 커밋한다).
            **실행: 월 1회 A2a 증분 뒤 — 10월 1일 점검에 부분 익절 그림자와 함께.**
            ...
            **및 같은 7개의 타이머**. 알림 유닛 자신에겐 안 건다(무한루프).
            ...
            ★ **Actions 실패(`notify-failure.yml`)도 같은 방으로 옮겼다** — 두 갈래로
            갈린다: 실패(VM 유닛 6 + 워크플로 6)는 그룹, 콘텐츠(뉴스·공시·일일분석·
            장중급등락·로그인)는 1:1 대화. 그룹에 뭐가 뜨면 무조건 고장이라는 뜻이다.
            ...
            (허용목록·주문당/일일 한도·가격밴드·공매도 금지)를 통과. 회귀 84건+변이 테스트 4건, 모의 서버 읽기 전용 dry-run 성공.
            **실전 TR_ID 는 공식 문서로 미확인**(분류기가 문서 열람을 막음) — 사용자가 대조 후 `live.tr_ids_reviewed`. Claude 는 키를
            입력·열람하지 않고 실계좌 모드를 켜지 않는다. 분류기가 처음 막았다가 사용자 재승인(Write 도구)으로 구현.
            ★ 같은 날 **폰용 읽기 전용 웹 화면 가동**(https://wonithink-stock.duckdns.org/autotrader/ — 기존 nginx 의 경로 하나만 추가,
            비밀번호+인증앱(TOTP), 로그인 한 번으로 상세, 유휴 30분) + **텔레그램 로그인 알림**(별도 프로세스, 토큰은 웹이 못 봄). 웹·알림 회귀
            67+23건. 패스키(지문)는 미착수. 자동 실행 타이머는 미설치(지금 autotrader 는 주문을 안 낸다). 세부: 세션인수인계-2026-09-20.md.
            ★ 2026-09-19 KOSPI200 선물 장중 신호 스터디(시가창·갭·ORB·VWAP 10셀,
            사전등록 bbcd55c) **전부 REJECT** — 주식 초단기가 비용에서 죽었으니 비용 1.4bp
            인 선물에선 살아날 것이라는 가설이 지지되지 않았다(gross 신호 자체가 없음,
            일 단위 셀 최소검출효과 ≈40bp). 재개 조건: 표본 2배 이상 또는 횡단면 새 가설.
            같은 날 무한매수법 취소·재접수 구현(위 "다음" 참고). 세션인수인계-2026-09-19.md.
            ...
            isolated 마진 1~2.5%로는 실측 BTC 경로에서 14일 만에 청산된다.
            funding 자체는 수익원(haircut50%에도 +6%)이지만 이 자본구조로는
            실현 불가 — 재도전 조건 없음(Binance Portfolio Margin 경로는
            크립토 실주문 인프라 자체가 없어 별도 스코프, 미착수).
            (2) 무한매수법 1a confirm_days 스윕(1~20일, 사용자 요청) —
            "지연을 줄이면 급락 초반 손실이 준다" 가설 기각(급락국면 성과가
            거의 무관, -37%대 그대로). **원형(2일) 유지, 재론 안 함.**
            (3) 반도체 산업사이클 연구축(SOXL·KR메모리주) 종료 — VIX·TSMC
            월매출·FRED PPI(WPU1178·PCU3344133441 둘 다, 인용오류 정정
            재검증)·NAND·복합구조(HBM대용) 전부 **REJECT**, DART 분기실적·
            IPG3344S는 표본부족으로 **HOLD**. 관세청 HS 8542.32(디램)
            수출단가만 통과했으나(다중검정보정 후에도 시간축 증거는 보강)
            000660(SK하이닉스)의 후반(2020-)한정 국면의존성이 반분점 우연·
            2026년 극값 착시 둘 다 아님을 재확인(3등분 점진강화·rolling
            OOS·2026 제외 n=140 유의 유지)한 뒤, 사전지정 문턱값(원
            finding의 quintile 재사용, 새 탐색 없음)으로 실제 룰까지
            백테스트 — **005930은 Sharpe·MDD 개선 성립, 000660은 전체기간
            Sharpe 오히려 악화**(전반에 그대로 적용한 대가, 국면의존성이
            실손익으로 재현됨). **사용자가 DRAM 연구 라인 전체를 HOLD로
            동결 확정** — 재개 후보(005930 전용/000660 국면필터/OOS 축적)는
            남기되 다음 세션이 임의로 이어가지 않는다.
            (4) futures Stage 6 목표변동성 연속sizing(Stage5-3 §9 후속)
            **REJECT** — 이산(0/0.5/1x)이 CAGR·Sharpe·MDD 다 개선했던
            2018-2024에서 연속은 Sharpe를 오히려 음전환시킨다. 무비용
            재계산으로 비용 기여 5bp/yr 이하 확인 — 원인은 "꼬리(Q5)에서만
            검증된 신호를 전구간 선형관계로 확장한" 설계 문제로 추정
            (미검증, 재개조건: 꼬리 내부 연속조정 등 새 가설).
            ...
            ★ `shares-snapshot` 30분 타임아웃 **닫힘**(2026-09-11, 같은 날 재측정).
            원인은 실패율이 아니었다 — 실패가 산발하지 않고 **어느 지점까지 실패 0,
            그 뒤로 100%** 다(112/166/214/233 에서 정지). 벽이지 비율이 아니고
            벽은 뒤로 밀리는 중이다. 2차 패스는 완주한 유일한 실행에서 **139 시도
            성공 0** 이라 껐다(재개는 `--passes 2`). 잘린 두 실행은 느린 게 아니라
            **매달렸다**(패스2 가 25분/16분간 로그 0줄) — pykrx 가 timeout 없이
            requests 를 부른다. 진짜 손해는 타임아웃이 아니라 **패스1 이 건진
            233종목까지 버려지는 것**(cancelled → commit skip). ★ `socket.
            setdefaulttimeout` 은 안 듣는다(urllib3 2.x, 실측) — `HTTPAdapter.send`
            에 넣는다. notify-failure 가 이제 `cancelled` 도 세고 이 워크플로도
            감시한다. ★★ **벽의 정체도 같은 날 확정**(프로브 2회, 문서 §6) —
            막힌 응답은 `200 OK` + `blockError_01.jsp` 다(429 가 아니라 **차단
            페이지**, 교훈81 그대로). 막히면 **로그인조차 안 된다**(로그인 URL 도
            같은 차단 페이지) → 세션 단위 기각. 5분 뒤 다른 실행이 124건을
            성공시켰다 → 계정 단위도 기각. **출처(IP) 단위**다. 그래서 2차 패스의
            원래 근거(353→167→272→273 "쉬면 회복")가 **confounded 였다** — 별개
            Actions 실행 = 새 러너 = 새 예산이었지 회복이 아니다. 그 위에 얹혀
            있던 "간격을 늦춰도 같은 벽" 추론도 같이 무너져 **속도를 늦추는 쪽이
            미검증 후보로 되살아났다**(1~3분에 100~230건이면 걸린다).
            ★★ **그 실험(SLEEP_SECONDS 0.2→0.6~1.0)은 2026-09-20 에 닫았다 —
            돌리지 않는다.** 전제가 만료됐다. (1) 30분 매달림은 09-11 수정 이후
            **재발 0건**: 09-15~09-18 평일 4회 연속 성공, 매회 4~5분.
            (2) 남은 실패 1건(09-14)은 **KRX 로그인 단계**다 — `JSONDecodeError:
            Expecting value: line 1 column 1`(= JSON 대신 차단 HTML) 로 재시도 4회
            전부 실패했고 3분 31초에 깨끗이 죽었다. **`SLEEP_SECONDS` 는 종목별
            수집 루프 안에서만 쓰인다**(build-shares-snapshot.py:208). 로그인은
            pykrx import 시점의 별도 재시도 루프다(:148~159) — 그 실패는 수집
            루프에 닿지도 못한다. 간격을 늦춰도 이 실패 모양은 안 변한다.
            지금 상태는 건강하다: 빠르게 실패 · carry_forward 가 받음 ·
            notify-failure 가 감시(이제 장애 전용 방으로 온다) · 데이터 최신
            (asOf 20260916). 재개 조건: **수집 루프 도중** 벽에 걸리는 실패가
            다시 나올 때. 로그인 차단이 잦아지면 그건 별개 문제(재시도 횟수·백오프).
            세부: docs/operations/shares-snapshot-timeout-2026-09.md
            ★★ **cron 주석의 KST 시각은 예정이지 실제가 아니다**(2026-09-11 전수
            실측, 같은 문서 §7.1) — 이 저장소의 모든 schedule 이 **4~6시간 밀려서**
            돈다(지연 +243~+360분). 데이터는 안 틀리지만 워크플로 간 순서 보장이
            우연에 기댄다 — 진짜 보장은 workflow_run 체인이다. 그리고 고빈도 cron 은
            **거의 전부 드롭된다**: `intraday-alert` 가 한국장 창에서 하루 42회
            기대인데 8거래일 전부 **실측 1회**다(§7.2). cron 으로는 못 고친다.
            ★★ 그 드롭이 이미 무엇을 망가뜨렸는지 재니 **알림이 한 번도 배달된
            적이 없었다**(2026-09-11). 사슬: 하루 1회 → '직전 체크'가 24시간 전 →
            24h 변화에 3% 임계 = 353종목 중 **269종목 발화** → 한 메시지 133줄 →
            텔레그램 **HTTP 400**(4096자 초과) → 게다가 `lastAlertAt` 을 전송
            **전에** 찍어 배달 안 된 알림이 쿨다운만 먹고 → 실패를 '채널 미설정'
            으로 오진(채널은 09-09 에 등록돼 있다). 워크플로는 내내 초록이었다.
            `detect()` 주석은 "순수 함수 - 테스트 가능"인데 테스트가 없었다.
            **고친 방향은 cron 이 아니라 규칙이다**(VM 이관은 막힌다 — 이 워크플로가
            `intraday-state.json` 을 커밋해서 VM 자격증명 금지 규칙과 충돌한다):
            `suddenMaxGapMinutes 30`(간격 밖이면 판정 안 함, 교훈57) ·
            메시지에 실제 경과분 표기 · `maxAlertsPerMessage 40`(생략 건수 공개) ·
            쿨다운 도장은 **전송 성공 뒤에만** · 전송실패≠채널미설정 구분.
            실제 상태 353종목 재현 269건 → **0건**. 지금 빈도에선 suddenMove 가
            잠들고 dailyMove 만 돈다 — 빈도가 회복되면 스스로 깨어난다(그래서 규칙을
            안 지웠다). 회귀 `scripts/test-intraday-check.js` 10건 신설(없던 것).
