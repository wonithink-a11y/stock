...
## 0. Gate Contract Verified (2026-08-04) — A0.7·A1a·A1b·A2a 완료(2026-08-05), 다음은 A2b 정찰
...
**다음은 A2b 커버리지 정찰**이다. 구현이 아니라 계약 정의가 먼저다 — 소스 한계가
설계를 좌우할 가능성이 크므로 커버리지 실측 전에는 계약을 고정하지 않는다.
...
트랙 A (10년 백필)
  ...
  A2b   가격 (폐지분)        커버리지 정찰 선행. 구현은 그 뒤   ← 다음 작업
  A3~A9                      미착수
```
...
9. A2b 커버리지 정찰 (구현 아님. 확보율 5개 분포 → A5 게이트 X% 확정)   ← 지금 여기
10. A2b 구현 · A3~A9
```
