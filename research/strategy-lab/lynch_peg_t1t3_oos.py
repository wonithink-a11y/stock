#!/usr/bin/env python
"""quality_factor_precheck_absolute.py의 Lynch PEG 결과(decile IC t=2.19,
고/저유동성 둘 다 플러스 - tercile 버그로 가려졌던 신호일 가능성) 후속.
PER 재검증(per_factor_t1t3_oos.py)과 완전히 같은 두 검증 - T1/T3 유동성
사후분해(반전 여부 재확인) + TRAIN/VALID/TEST(60/15/25 월별 시간분할) OOS.

quality_factor_precheck_absolute.py의 데이터 로드·패널 구성 함수를 그대로
재사용 - 새 계산·API 호출 없음.

  python lynch_peg_t1t3_oos.py
"""
import json
import os
...
from quality_factor_precheck_absolute import (
    REPO_ROOT, load_peg_lookup, build_panel, monthly_rebalance_dates,
...
def t1t3_decomposition(df):
    ...
    for m, g in df.groupby("entry_date"):
        picks.append(g.sort_values("peg", ascending=True).head(TOP_N))
...
def main():
    peg_lookup = load_peg_lookup()
    tickers = sorted({t for t, _ in peg_lookup.keys()})
    calendar = TradingCalendar(repo_root=REPO_ROOT)
    a2a = A2aProvider(repo_root=REPO_ROOT, use_cache=True)
    bars_raw = a2a.load(tickers, START, END, universe_hash="peg-precheck-absolute")
    bars_by_ticker = {t: _drop_suspension_rows(df) for t, df in bars_raw.items()}
    rebalance_dates = monthly_rebalance_dates(calendar, START, END)
    df = build_panel(bars_by_ticker, rebalance_dates, peg_lookup, "peg")
    print(f"panel rows={len(df)}, months={df['entry_date'].nunique()}")

    print("\n=== T1/T3 유동성 분해 (top-30 저PEG, 유동성필터 없음, 사후 진단) ===")
    ...
    for name, (sub, months) in splits.items():
        bt = run_backtest(sub, "peg", "low", liquidity_filter=None)
        ic = decile_ic(sub, "peg", "low")
        ...
        print(f"  {name} ({oos[name]['months']}, {len(months)}mo): "
              ...
              f"decileIC spread={ic['meanSpread(bestDecile-worstDecile)']} t={ic['t']} (n={ic['nMonths']}mo)")

    out_dir = os.path.join(REPO_ROOT, "research", "strategy-lab", "reports",
                            "2026-08-28-quality-factor-precheck-absolute")
    out_path = os.path.join(out_dir, "peg-t1t3-oos.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({
            "context": "quality_factor_precheck_absolute.py(peg, decile IC t=2.19) 후속 - "
                       "T1/T3 유동성 사후분해(반전 재확인) + TRAIN/VALID/TEST OOS.",
            "t1t3": t1t3, "oos": oos,
