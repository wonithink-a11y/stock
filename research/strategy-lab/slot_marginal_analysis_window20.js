 ...
 *
 * node research/strategy-lab/slot_marginal_analysis_window20.js
 *
 * 접근 (production 무수정):
 ...
 *
 * 출력: research/strategy-lab/findings/slot-marginal-contribution-window20/snapshots.json
 *
 * (window20 변형: SD5D_WINDOW=20 — 수급 트렌드를 20거래일 창으로 측정)
 */
'use strict';
const fs = require('fs');
...
function main() {
  ...
  const outDir = path.join(ROOT, 'research', 'strategy-lab', 'findings', 'slot-marginal-contribution-window20');
  fs.mkdirSync(outDir, { recursive: true });
  const outPath = path.join(outDir, 'snapshots.json');
