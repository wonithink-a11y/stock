"""A2b price adapter - reads data/backfill/price/a2b/*.jsonl.gz read-only.

Same schema and scan strategy as A2aProvider (mirrors it deliberately - the two
providers differ only in which directory/manifest they read). A2b also has two
non-year files in the same directory (delisted-exit.jsonl.gz,
price-quality-excluded.jsonl.gz) - the digit-prefix filename check that
A2aProvider already uses to skip price-quality-excluded.jsonl.gz skips both here
too, no extra logic needed.
...
A2B_DIR = "data/backfill/price/a2b"
A2B_MANIFEST = "data/backfill/manifest/A2b.json"
CACHE_DIR = "research/strategy-lab/.cache/a2b"
...
class A2bProvider(PriceProvider):
    def __init__(self, repo_root=".", use_cache=True):
        ...
        with open(self.repo_root / A2B_MANIFEST, encoding="utf-8") as f:
    ...
    def _scan(self, tickers, start, end):
        """Single sequential pass over the year-files - same requirement as A2a."""
        buffers = {t: [] for t in tickers}
        a2b_dir = self.repo_root / A2B_DIR
        for path in sorted(a2b_dir.glob("*.jsonl.gz")):
            if not path.name[:4].isdigit():
                continue  # skips delisted-exit.jsonl.gz, price-quality-excluded.jsonl.gz
