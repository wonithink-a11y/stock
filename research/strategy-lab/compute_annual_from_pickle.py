#!/usr/bin/env python
"""Load existing full_smoke_result.pkl and compute annual analysis for 2,154 trades."""
import sys
import os
import json
import pickle
...
def main():
    print("Loading existing full_smoke_result.pkl...")
    ...
    print(f"First signal: {result['diag'].get('firstSignalDate')}")
    print(f"Last signal: {result['diag'].get('lastSignalDate')}")
    ...
    output = {
        ...
        "runClass": "SMOKE",
        "dataSource": "runner full smoke 2026-08-15 post-fix (2,154 trades) - loaded from pickle",
        "annual": annual,
    }
    ...
    # Overall stats from postfix report
    print("\n--- Overall (from 2,154 trades) ---")
    print(f"Total trades: 2154")
    print(f"Win rate: 24.88%")
    print(f"Profit factor: 0.7825")
    print(f"CAGR: -12.25%")
    print(f"MDD: -83.34%")
    print(f"Avg holding period: 13.69 days")
