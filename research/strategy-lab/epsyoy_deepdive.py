#!/usr/bin/env python
"""eps_yoy Deep-Dive Verification — KR Research Lab (2026-08-30).

Tests eps_yoy 12M forward return independence:
1. KOSPI / KOSDAQ split
2. Size terciles (dv20_log)
3. Year-by-year Top 10% returns
4. Valuation control (PBR, PER residualization)
5. Neutralized eps_yoy
"""
import bisect
import gzip
import json
...
OUT_DIR = os.path.join(LAB, "reports", "2026-08-30-epsyoy-deepdive")
...
def decile_analysis(sub, f, fwd_col):
    ...
    ns = []
    top10_yearly = {}  # track top decile yearly returns
...
def main(max_tickers=None):
    ...
    # ---- Neutralize eps_yoy against PBR and PER (cross-sectional regression) ----
    print("neutralizing eps_yoy vs PBR/PER ...", flush=True)
    base["eps_yoy_resid"] = np.nan
    for ddate, g in base.groupby("date"):
        mask = g["eps_yoy"].notna() & g["pbr"].notna() & g["per"].notna()
        if mask.sum() < 50:
            continue
        X = g.loc[mask, ["pbr", "per"]].values
        y = g.loc[mask, "eps_yoy"].values
        # add intercept
        X = np.column_stack([np.ones(len(X)), X])
        try:
            beta = np.linalg.lstsq(X, y, rcond=None)[0]
            resid = y - X @ beta
            base.loc[g.index[mask], "eps_yoy_resid"] = resid
    ...
    # ---- Analysis ----
    fwd_col = "fwd12M"
    results = {"experiment": "EPS_YOY_DEEPDIVE-2026-08",
    ...
    # 1. Overall
    print("\n=== Overall ===")
    sub = base[["ticker", "date", "market", "period", fwd_col, "eps_yoy", "size_tercile", "pbr", "per"]].dropna(subset=["eps_yoy", fwd_col])
    res = decile_analysis(sub, "eps_yoy", fwd_col)
    ...
    for mkt in ["KOSPI", "KOSDAQ"]:
        ...
        if len(sub_m) == 0:
            continue
        res = decile_analysis(sub_m, "eps_yoy", fwd_col)
    ...
    for sz in ["Small", "Mid", "Large"]:
        ...
        if len(sub_s) == 0:
            continue
        res = decile_analysis(sub_s, "eps_yoy", fwd_col)
    ...
    # 4. Valuation-controlled: eps_yoy residual
    print("\n=== Valuation-Controlled (eps_yoy_resid) ===")
    sub_r = base[["ticker", "date", "market", "period", fwd_col, "eps_yoy_resid"]].dropna(subset=["eps_yoy_resid", fwd_col])
    if len(sub_r) > 0:
        res = decile_analysis(sub_r, "eps_yoy_resid", fwd_col)
        res["nObs"] = int(len(sub_r))
        results["analyses"]["eps_yoy_resid_vs_pbr_per"] = res
        sp = res["spread"]
        print(f"  n={res['nObs']} Q10-Q1={sp['mean']:.4f} t={sp['t']} posYR={sp['posYearRatio']} IC_t={res['ic'].get('t')}")

    # 5. Double sort: eps_yoy quintile x pbr quintile
    print("\n=== Double Sort: eps_yoy x PBR ===")
    sub_ds = base[["ticker", "date", "market", "period", fwd_col, "eps_yoy", "pbr"]].dropna(subset=["eps_yoy", "pbr", fwd_col])
    if len(sub_ds) > 0:
        ds_results = {}
        for ddate, g in sub_ds.groupby("date"):
            if len(g) < MIN_NAMES:
                continue
            g2 = g.copy()
            g2["eps_q"] = pd.qcut(g2["eps_yoy"].rank(method="first"), 5, labels=False) + 1
            g2["pbr_q"] = pd.qcut(g2["pbr"].rank(method="first"), 5, labels=False) + 1
            m = g2.groupby(["eps_q", "pbr_q"])[fwd_col].mean()
            if (5, 1) in m.index and (1, 5) in m.index:
                ds_results.setdefault("high_eps_low_pbr", []).append(float(m.loc[(5, 1)]))
                ds_results.setdefault("low_eps_high_pbr", []).append(float(m.loc[(1, 5)]))
            if 5 in g2["eps_q"].values and 1 in g2["eps_q"].values:
                top = g2[g2["eps_q"] == 5][fwd_col].mean()
                bot = g2[g2["eps_q"] == 1][fwd_col].mean()
                ds_results.setdefault("eps_q5_q1", []).append((ddate, float(top - bot)))
        if ds_results:
            for k, v in ds_results.items():
                if k in ["high_eps_low_pbr", "low_eps_high_pbr"]:
                    arr = np.array(v)
                    ...
                    print(f"  eps_q5-q1 (pbr controlled): mean={arr.mean():.4f} t={t:.2f} posYR={posYR:.2f} n={len(arr)}")
            results["analyses"]["double_sort_eps_pbr"] = {k: (float(np.mean(v)), int(len(v))) if isinstance(v[0], float) else v for k, v in ds_results.items()}
    ...
    out_path = os.path.join(OUT_DIR, "epsyoy-deepdive-results.json")
