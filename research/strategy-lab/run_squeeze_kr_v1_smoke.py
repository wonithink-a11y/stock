#!/usr/bin/env python
"""SQUEEZE-KR-V1 smoke driver (STEP 1).

Mirrors run_macd_kr_v1_smoke.py structure exactly:
...
- State-machine entry/exit in build_events (dynamic momentum zero-cross NOT expressible in executor)
...
- Benchmarks: Equal-Weight daily-rebalanced, Buy & Hold (same universe)
- Output: research/strategy-lab/findings/squeeze-kr-v1-smoke/smoke_result.json
"""
...
OUT_DIR = REPO_ROOT / "research" / "strategy-lab" / "findings" / "squeeze-kr-v1-smoke"
...
def build_events(symbol: str, feats: pd.DataFrame, cal: TradingCalendar, listed_at: str | None):
    entry_sig = feats.get("release", pd.Series(False, index=feats.index)) & (feats.get("momentum", 0) > 0)
    exit_sig = feats.get("exit_cross", pd.Series(False, index=feats.index))
    dates = feats.index[entry_sig | exit_sig]

    events = []
    audit = {"pre_listing_signals": 0, "release_total": int(feats["release"].sum()),
...
def closes_last(t: dict) -> pd.Timestamp:
    return pd.Timestamp(t["exit_date"])
...
def run_subset(n_tickers: int, seed: int = 42):
    ...
    # Batch load all bars (like MACD driver)
    ...
    # Build master calendar dates for benchmarks (MACD parity)
    master_dates = [d for d in cal.days if PERF_START <= d <= END]
    master_ts = [pd.Timestamp(d) for d in master_dates]

    all_trades = []
    all_audit = {"pre_listing_signals": 0, "release_total": 0,
                 "warmup_excluded": 0, "entry_count": 0, "exit_count": 0}
    ...
    result = {
        ...
        "indicators": {
            "bollinger": {"length": BB_LEN, "mult": BB_MULT, "stdDev": "population ddof=0"},
            "keltner": {"length": KC_LEN, "mult": KC_MULT, "rangeMa": "SMA(TR,20)"},
            "momentum": {"lookback": MOM_LEN, "method": "linreg_endpoint"}
        },
    ...
    print("\nSQUEEZE-KR-V1 STEP 1 SMOKE driver ready.")
