"""dd252_v1 최소 자체검증 - lowmom60_v1과 동일 패턴(strategies/base.py
...
_RULE_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                           "strategies", "dd252_v1", "rule.py")
_spec = importlib.util.spec_from_file_location("strategies.dd252_v1.rule", _RULE_PATH)
...
def test_selection_dd252_ranking():
    """Locks in Arm A design: every selection came from a valid dd_252_skip1m month,
    and the promised top-30-by-highest-dd cap held (no liquidity filter in Arm A)."""
...
def run():
    ...
    test_selection_dd252_ranking()
    print("test_dd252_v1: OK")
