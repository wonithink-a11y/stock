"""engine/live/bithumbPaperBroker.py 구조 테스트 - 네트워크 없음(FakeClient로
BithumbClient.get_ticker를 대체). test_upbit_paper_broker.py와 동일 구조."""
import os
import sys

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from engine.live.bithumbPaperBroker import BithumbPaperBroker
...
def test_submit_and_check_fill_round_trip():
    broker = BithumbPaperBroker(client=FakeClient(50000000.0))
    order_no = broker.submit_buy("KRW-BTC", 0.001)
    result = broker.check_fill(order_no, "20260914", 0.001)
...
def test_stateless_across_broker_instances():
    ...
    order_no = BithumbPaperBroker(client=client).submit_sell("KRW-ETH", 2.0)
    fresh_broker = BithumbPaperBroker(client=client)  # 새 인스턴스 = 재시작 흉내
    result = fresh_broker.check_fill(order_no, "20260914", 2.0)
...
def test_current_price_delegates_to_client():
    broker = BithumbPaperBroker(client=FakeClient(999.0))
