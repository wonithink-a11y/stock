#!/usr/bin/env python
"""10-KR-9: Market Breadth / Relative Strength 최소 검증.

Relative Strength (cross-sectional, per stock):
  RS60 = mom60 - market_ret60      (stock 60D return minus market 60D return)
  market_ret60 = cross-sectional mean of mom60 on each date (equal-weight)

Market Breadth (market-level time series):
  adv_pct      = daily breadth (fraction of stocks with positive daily return)  [reuse breadth.parquet]
  adv_pct_20d  = rolling 20D mean of adv_pct (built here)
...
BREADTH_PATH = os.path.join(REPO_ROOT, "research", "strategy-lab", "data", "market-regime", "breadth.parquet")
OUT_DIR = os.path.join(REPO_ROOT, "research", "strategy-lab", "reports", "2026-08-28-kr-breadth-rs")
...
def main():
    ...
    # market 60D return: cross-sectional mean of mom60 per date
    mkt60 = df.groupby("date")["mom60"].transform("mean")
    df["RS60"] = df["mom60"] - mkt60
    # controls
    ta = df["total_amount"].replace(0, np.nan)
    df["foreign_ratio"] = df["foreign_net"] / ta
    df["inst_ratio"] = df["inst_net"] / ta
    # vol control (from A4 close log returns, 20D std)
    ...
    # market forward: need a per-date market series for breadth test; keep avg stock fwd
    df["avg_stock_ret5"] = df.groupby("date")["fwd5"].transform("mean")
    df["avg_stock_ret20"] = df.groupby("date")["fwd20"].transform("mean")

    df = df.dropna(subset=["RS60"])
    ...
    result = {"experiment": "10-KR-9: Market Breadth / Relative Strength", "horizons": H,
              "data": {"rows": len(df), "tickers": df["ticker"].nunique()}}

    # --- Relative Strength (cross-sectional) ---
    print("\n=== Relative Strength (RS60 = mom60 - market60) ===")
    result["RS60"] = {}
    for pname in ["TRAIN", "VALID", "TEST"]:
        rs_q = periods[pname]
        result["RS60"][pname] = {}
        for hn, hc in H.items():
            sp = quintile_spread(by_date, "RS60", hc, rs_q)
            ic = daily_ic(by_date, "RS60", hc, rs_q)
            result["RS60"][pname][hn] = {"quintile": sp, "ic": ic}
            print(f"  {pname} {hn}: Q5-Q1={sp['pooledQ5minusQ1']}, IC={ic['icMean']}(t={ic['icT']})")

    # mom60 reference (raw momentum for contrast)
    print("\n=== Reference: mom60 ===")
    result["mom60_ref"] = {}
    for pname in ["TRAIN", "VALID", "TEST"]:
        rs_q = periods[pname]
        result["mom60_ref"][pname] = {}
        for hn, hc in H.items():
            sp = quintile_spread(by_date, "mom60", hc, rs_q)
            ic = daily_ic(by_date, "mom60", hc, rs_q)
            result["mom60_ref"][pname][hn] = {"quintile": sp, "ic": ic}
            print(f"  {pname} {hn}: Q5-Q1={sp['pooledQ5minusQ1']}, IC={ic['icMean']}(t={ic['icT']})")

    # --- Residual: RS vs mom60 / vol+flow ---
    print("\n=== Residual IC RS60 | mom60 ===")
    result["RS60_resid_mom60"] = {}
    for pname in ["TRAIN", "VALID", "TEST"]:
        rs_q = periods[pname]
        result["RS60_resid_mom60"][pname] = {}
        for hn, hc in H.items():
            s = orthogon_ic(by_date, "RS60", ["mom60"], hc, rs_q)
            result["RS60_resid_mom60"][pname][hn] = s
            print(f"  {pname} {hn}: residIC={s['icMean']}(t={s['icT']})")

    print("\n=== Residual IC RS60 | mom60+rv20+foreign+inst ===")
    result["RS60_resid_all"] = {}
    ctl_all = ["mom60", "rv20", "foreign_ratio", "inst_ratio"]
    for pname in ["TRAIN", "VALID", "TEST"]:
        rs_q = periods[pname]
        result["RS60_resid_all"][pname] = {}
        for hn, hc in H.items():
            s = orthogon_ic(by_date, "RS60", ctl_all, hc, rs_q)
            result["RS60_resid_all"][pname][hn] = s
            print(f"  {pname} {hn}: residIC={s['icMean']}(t={s['icT']})")
    ...
    if os.path.exists(BREADTH_PATH):
        ...
        br = br.dropna(subset=["adv_pct", "usableFromDate"])
        breadth = br.set_index("date")[["adv_pct"]].copy()
        breadth["adv_pct_20d"] = breadth["adv_pct"].rolling(20).mean()
        # PIT: breadth value computed from t-1 close usable at t
        breadth["adv_pct"] = breadth["adv_pct"].shift(1)
        breadth["adv_pct_20d"] = breadth["adv_pct_20d"].shift(1)
        print(f"  breadth rows: {len(breadth)}")
        result["breadth_data"] = {"rows": len(breadth), "period": [br["date"].min(), br["date"].max()]}

        # forward market/avg-stock returns per date
        mktfwd = df[["date", "avg_stock_ret5", "avg_stock_ret20"]].drop_duplicates("date").set_index("date")
        b = breadth.join(mktfwd, how="inner")
        # Breadth quintile vs forward avg-stock return (time-series)
        result["breadth"] = {}
        for bfeat in ["adv_pct", "adv_pct_20d"]:
            print(f"\n  -- {bfeat} (daily, market time series) --")
            result["breadth"][bfeat] = {}
            for hn, hc in [("5D", "avg_stock_ret5"), ("20D", "avg_stock_ret20")]:
                row = {pname: {"period": None} for pname in ["TRAIN", "VALID", "TEST"]}
                ps = {"TRAIN": [], "VALID": [], "TEST": []}
                for d, r in b.iterrows():
                    pd_label = period_of(d)
                    if pd_label not in ps or pd.isna(r[bfeat]) or pd.isna(r[hc]):
                        continue
                    ps[pd_label].append((d, r[bfeat], r[hc]))
                # within-period tercile of breadth
                for pname, items in ps.items():
                    if len(items) < 10:
                        row[pname] = {"n": len(items), "tercile": None}; continue
                    bvals = np.array([v for _, v, _ in items])
                    lo, hi = np.quantile(bvals, [1/3, 2/3])
                    r_low, r_hi = [], []
                    for d, bv, rv in items:
                        if bv <= lo: r_low.append(rv)
                        elif bv >= hi: r_hi.append(rv)
                    def ts(x):
                        x = np.array(x); return round(float(np.mean(x)), 5) if len(x) else None
                    row[pname] = {"n": len(items), "lowBreadth_fwdRet": ts(r_low), "highBreadth_fwdRet": ts(r_hi),
                                  "low_n": len(r_low), "high_n": len(r_hi)}
                    print(f"  {bfeat} {pname} {hn}: n={len(items)} lowB={row[pname]['lowBreadth_fwdRet']} "
                          f"highB={row[pname]['highBreadth_fwdRet']}")
                result["breadth"][bfeat][hn] = row

    # --- Portfolio: long top-Q5 RS60 ---
    print("\n=== Portfolio (long top-Q5 RS60, monthly, net 30bps) ===")
    portfolio = {}
    close_ser = df[["date", "ticker", "close"]].copy()
    close_ser["close_20"] = close_ser.groupby("ticker")["close"].shift(-20)
    for axis, feat in [("RS60", "RS60"), ("mom60_ref", "mom60")]:
        portfolio[axis] = {}
        for pname in ["TRAIN", "VALID", "TEST"]:
    ...
    out_path = os.path.join(OUT_DIR, "kr-breadth-rs-results.json")
