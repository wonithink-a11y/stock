"""Single-factor backtest for rv60_pct (low decile).
...
def hold_sessions(symbol: str, as_of: str):
    ...
    백테스트는 generate_signals -> risk_spec_for 경로로 이 값을 쓰는데, 페이퍼
    엔진(poll_once)에는 그 경로가 없어 정책의 고정 maxHoldingSessions(21)를 썼다.
    2026년 리밸런싱일 9개 중 5개가 22~23세션이라, 고정 21이면 다음 리밸런싱
    1~2세션 전에 시간청산으로 팔고 곧바로 다시 사는 헛회전이 난다(백테스트에
    없는 왕복비용 30~60bp). 2026-09-09 확인.
    """
