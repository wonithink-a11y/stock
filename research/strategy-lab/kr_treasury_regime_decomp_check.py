#!/usr/bin/env python
"""kr_treasury_regime_decomp_check.py — "Treasury_Top20" 전략이 보고한 CAGR 중
실제로 Treasury 선택(Risk-On 구간)이 낸 몫과 LOWMOM60 대체분(Neutral/Risk-Off
구간)이 낸 몫을 분리한다.

배경: kr_treasury_incremental_step1.py를 직접 읽어보니 "Treasury_Top20"이라는
이름과 달리 Risk-On 구간에서만 TreasuryRatio Top20%를 쓰고, Neutral/Risk-Off
구간에는 LOWMOM60으로 대체한다(w_treas = 1.0 if regime=='Risk-On' else 0.0).
TRAIN 기간(2016~2022.6) 중 Risk-On 비중은 일별로 약 31.7%뿐이라, 보고된
"TRAIN CAGR 13.80%"의 상당 부분이 실은 Treasury 선택이 아니라 LOWMOM60
대체분일 가능성이 있다. 원본과 완전히 동일한 로직(run_strategy 그대로
재사용 대신 복제 - 계측 코드만 추가)으로 재실행해 두 구간을 분리 집계한다.
"""
import gzip
import json
import os
import time

import numpy as np
import pandas as pd

REPO_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
A4_PATH = os.path.join(REPO_ROOT, "research", "strategy-lab", "data", "a4", "a4-research-dataset.parquet")
A3C_DIR = os.path.join(REPO_ROOT, "data", "backfill", "fundamentals", "a3c")
REGIME_PATH = os.path.join(REPO_ROOT, "research", "strategy-lab", "data", "market-regime", "regime_labels.parquet")
OUT_DIR = os.path.join(REPO_ROOT, "research", "strategy-lab", "reports", "2026-09-02-kr-treasury-regime-decomp")

TRAIN_END = "2022-06-30"
VALID_END = "2024-01-01"
TOP_N = 30
MIN_NAMES = 30
COST_BPS = 15.0
ROUNDTRIP_BPS = 2 * COST_BPS
MIN_TURNOVER = 100_000_000.0
REGIMES = ["Risk-On", "Neutral", "Risk-Off"]


def period_of(d):
    return "TRAIN" if d <= TRAIN_END else ("VALID" if d <= VALID_END else "TEST")


def monthly_reb(dates):
    out, seen = [], set()
    for d in sorted(dates):
        if d[:7] not in seen:
            seen.add(d[:7])
            out.append(d)
    return out


def quarterly_reb(dates):
    out, seen = [], set()
    for d in sorted(dates):
        q = d[:4] + "-Q" + str((int(d[5:7]) - 1) // 3 + 1)
        if q not in seen:
            seen.add(q)
            out.append(d)
    return out


def normd(s):
    s = str(s)
    if len(s) == 8 and s.isdigit():
        return f"{s[:4]}-{s[4:6]}-{s[6:]}"
    return s


def select_as_of(records, as_of):
    best = None
    for rec in records:
        af = normd(rec[0])
        if af > as_of:
            continue
        if best is None or rec[1] > best[1] or (rec[1] == best[1] and af > normd(best[0])):
            best = rec
    return best


def cagr_of(rets):
    if not rets:
        return None
    mr = np.array(rets)
    span = len(mr) / 12
    eq = float(np.prod(1 + mr))
    return eq ** (1 / max(span, 1e-9)) - 1 if eq > 0 else (1 + np.sum(mr)) ** (1 / max(span, 1e-9)) - 1


def main():
    os.makedirs(OUT_DIR, exist_ok=True)
    t0 = time.time()
    print("Loading A4...", flush=True)
    df = pd.read_parquet(A4_PATH, columns=["ticker", "date", "close", "total_amount", "total_volume"])
    df = df.drop_duplicates(subset=["ticker", "date"], keep="last")
    df["date"] = df["date"].astype(str)
    df = df.sort_values(["ticker", "date"]).reset_index(drop=True)

    df["mom60"] = df.groupby("ticker", sort=False)["close"].pct_change(60)
    df["turnover20"] = (df["close"] * df["total_volume"]).groupby(df["ticker"]).transform(
        lambda s: s.rolling(20, min_periods=20).mean())
    df = df.dropna(subset=["turnover20"])

    print("Loading raw A3c (treasuryRatio)...", flush=True)
    TREAS, ISSUED = {}, {}
    for y in range(2015, 2026):
        fp = os.path.join(A3C_DIR, f"{y}.jsonl.gz")
        if not os.path.exists(fp):
            continue
        with gzip.open(fp, "rt", encoding="utf-8") as f:
            for line in f:
                r = json.loads(line)
                if not str(r.get("periodEnd", "")).endswith("1231"):
                    continue
                t = r.get("ticker")
                if t is None:
                    continue
                af = normd(str(r["availableFrom"]))
                fy = int(r["fiscalYear"])
                try:
                    if r.get("istcTotqy") is not None:
                        TREAS.setdefault(t, []).append((af, fy, float(r["istcTotqy"])))
                    if r.get("isuStockTotqy") is not None:
                        ISSUED.setdefault(t, []).append((af, fy, float(r["isuStockTotqy"])))
                except (TypeError, ValueError):
                    pass

    def val(rm, t, as_of):
        cur = select_as_of(rm.get(t, []), as_of)
        return cur[2] if cur is not None else None

    all_dates = sorted(df["date"].unique())
    months = monthly_reb(all_dates)
    qmonths = quarterly_reb(all_dates)
    q_idx_map_full = {d: i for i, d in enumerate(qmonths)}

    base = df[df["date"].isin(months)][["ticker", "date", "mom60", "turnover20"]].copy()
    rows = []
    for (t, d), _ in base.groupby(["ticker", "date"]).size().items():
        tr = val(TREAS, t, d)
        isu = val(ISSUED, t, d)
        rows.append({"ticker": t, "date": d,
                     "treasuryRatio": tr / isu if (tr is not None and isu and isu != 0) else None})
    treas_panel = pd.DataFrame(rows)
    m = pd.merge(base, treas_panel, on=["ticker", "date"], how="left")
    m = m.dropna(subset=["treasuryRatio"])

    print("Loading regime labels...", flush=True)
    rl = pd.read_parquet(REGIME_PATH)
    rl = rl.dropna(subset=["usableFromDate"])
    rl["usableFromDate"] = rl["usableFromDate"].astype(str)
    regime_lut = rl.set_index("usableFromDate")["regime"]

    close_by_date = {d: gd.set_index("ticker")["close"] for d, gd in df.groupby("date")}
    liq = m[m["turnover20"] >= MIN_TURNOVER].copy()

    def get_regime_for_entry(sd):
        ent_d = next((x for x in all_dates if x > sd), None)
        if ent_d is None:
            return None
        return regime_lut.get(ent_d, None)

    print(f"  setup done ({time.time()-t0:.0f}s)", flush=True)

    # Treasury_Top20 원본 로직 그대로(kr_treasury_incremental_step1.py) 재현하되
    # 매달 어느 슬리브가 실제로 쓰였는지(sleeveUsed)를 같이 기록한다.
    results = {}
    for p in ["TRAIN", "VALID", "TEST"]:
        period_dates = [d for d in months if period_of(d) == p]
        period_qdates = [d for d in qmonths if d in period_dates]
        q_idx_map = {d: i for i, d in enumerate(period_qdates)}

        lowmom_sleeves, treas_sleeves = [], []
        monthly_rows = []

        for k, sd in enumerate(period_dates):
            regime = get_regime_for_entry(sd)
            if regime not in REGIMES:
                continue
            w_treas = 1.0 if regime == "Risk-On" else 0.0
            w_lowmom = 1.0 - w_treas

            this_low = liq[liq["date"] == sd].dropna(subset=["mom60"])
            if len(this_low) >= TOP_N:
                this_low = this_low.sort_values("mom60").head(TOP_N)
                lowmom_sleeves.append((k, set(this_low["ticker"].tolist())))

            if sd in q_idx_map and regime == "Risk-On":
                qk = q_idx_map[sd]
                this_tres = m[m["date"] == sd].dropna(subset=["treasuryRatio"])
                if len(this_tres) >= MIN_NAMES:
                    this_tres = this_tres.copy()
                    this_tres["q"] = pd.qcut(this_tres["treasuryRatio"].rank(method="first"), 5, labels=False, duplicates="drop")
                    picks = this_tres[this_tres["q"] == 4]["ticker"].tolist()
                    if picks:
                        treas_sleeves.append((qk, set(picks)))

            lowmom_sleeves = [(si, sp) for (si, sp) in lowmom_sleeves if k - si < 1]
            current_q_idx = q_idx_map.get(sd, -1)
            if regime == "Risk-On":
                treas_sleeves = [(si, sp) for (si, sp) in treas_sleeves if current_q_idx - si < 2]
            else:
                treas_sleeves = []

            if not lowmom_sleeves and not treas_sleeves:
                continue
            ent_d = next((x for x in all_dates if x > sd), None)
            if ent_d is None or k + 1 >= len(period_dates):
                continue
            ext_d = period_dates[k + 1]
            ent = close_by_date[ent_d]
            ext = close_by_date[ext_d]

            lowmom_ret = 0.0
            for _, picks in lowmom_sleeves:
                rets = [ext.loc[t] / ent.loc[t] - 1.0 for t in picks
                        if t in ent.index and t in ext.index and ent.loc[t] > 0]
                if rets:
                    lowmom_ret = w_lowmom * float(np.mean(rets))
            treas_ret = 0.0
            for _, picks in treas_sleeves:
                rets = [ext.loc[t] / ent.loc[t] - 1.0 for t in picks
                        if t in ent.index and t in ext.index and ent.loc[t] > 0]
                if rets:
                    treas_ret = w_treas * float(np.mean(rets))

            combined_ret = lowmom_ret + treas_ret
            net_ret = combined_ret - ROUNDTRIP_BPS / 10000
            monthly_rows.append({"date": sd, "regime": regime,
                                  "sleeveUsed": "Treasury" if regime == "Risk-On" else "LOWMOM60_fallback",
                                  "ret": net_ret})

        mdf = pd.DataFrame(monthly_rows)
        all_cagr = cagr_of(mdf["ret"].tolist()) if len(mdf) else None
        treas_only = mdf[mdf["sleeveUsed"] == "Treasury"]
        fallback_only = mdf[mdf["sleeveUsed"] == "LOWMOM60_fallback"]
        results[p] = {
            "nMonths": len(mdf),
            "riskOnMonths": len(treas_only), "riskOnPct": round(len(treas_only) / max(len(mdf), 1), 3),
            "fallbackMonths": len(fallback_only),
            "blendedCAGR": round(all_cagr, 4) if all_cagr is not None else None,
            "treasuryOnlyMonths_meanRet": round(float(treas_only["ret"].mean()), 4) if len(treas_only) else None,
            "treasuryOnlyMonths_cagrIfAlwaysOn": round(cagr_of(treas_only["ret"].tolist()), 4) if len(treas_only) else None,
            "fallbackMonths_meanRet": round(float(fallback_only["ret"].mean()), 4) if len(fallback_only) else None,
            "fallbackMonths_cagrIfAlwaysOn": round(cagr_of(fallback_only["ret"].tolist()), 4) if len(fallback_only) else None,
        }
        r = results[p]
        print(f"\n[{p}] n={r['nMonths']} Risk-On={r['riskOnMonths']}({r['riskOnPct']*100:.1f}%) "
              f"Fallback={r['fallbackMonths']}", flush=True)
        print(f"  blended(reported) CAGR = {r['blendedCAGR']*100:.2f}%" if r['blendedCAGR'] is not None else "  blended: n/a", flush=True)
        print(f"  Treasury-sleeve months only: mean monthly ret = "
              f"{r['treasuryOnlyMonths_meanRet']*100 if r['treasuryOnlyMonths_meanRet'] is not None else float('nan'):.2f}%, "
              f"annualized(if always-on) = {r['treasuryOnlyMonths_cagrIfAlwaysOn']*100 if r['treasuryOnlyMonths_cagrIfAlwaysOn'] is not None else float('nan'):.2f}%", flush=True)
        print(f"  LOWMOM60-fallback months only: mean monthly ret = "
              f"{r['fallbackMonths_meanRet']*100 if r['fallbackMonths_meanRet'] is not None else float('nan'):.2f}%, "
              f"annualized(if always-on) = {r['fallbackMonths_cagrIfAlwaysOn']*100 if r['fallbackMonths_cagrIfAlwaysOn'] is not None else float('nan'):.2f}%", flush=True)

    out_path = os.path.join(OUT_DIR, "kr-treasury-regime-decomp-results.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"purpose": "Treasury_Top20 전략의 TRAIN/VALID/TEST CAGR 중 "
                              "Risk-On(Treasury 선택) 몫과 Neutral/Risk-Off(LOWMOM60 대체) 몫 분리",
                   "results": results, "executionTime_s": round(time.time() - t0, 1)},
                  f, ensure_ascii=False, indent=1, default=str)
    print(f"\nSaved: {out_path}", flush=True)


if __name__ == "__main__":
    main()
