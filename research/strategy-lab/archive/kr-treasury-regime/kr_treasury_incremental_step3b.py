#!/usr/bin/env python
"""10-KR-23-24 STEP 3B: TreasuryRatio Incremental Portfolio Test - Top5%.

Tests 3 strategies with Treasury High5% cutoff:
1. LOWMOM60 baseline
2. Treasury Top20% baseline
3. LOWMOM 3-bucket × Treasury High5% (incremental)
"""
import gzip
import json
import os
...
def run_strategy(period_dates, period_qdates, q_idx_map, liq, m, close_by_date,
    ...
    for k, sd in enumerate(period_dates):
        ...
        # Strategy 3: LOWMOM 3-bucket × Treasury High5%
        elif strategy_name == "LOWMOM_Treasury_High5":
            w_treas = 1.0 if regime == "Risk-On" else 0.0
            w_lowmom = 1.0 - w_treas
        else:
        ...
        if sd in q_idx_map and regime == "Risk-On":
            ...
            if len(this_tres) >= MIN_NAMES:
                if strategy_name == "Treasury_Top20":
                    ...
                    # LOWMOM 3-bucket × Treasury High5%
                    this_tres = this_tres.copy()
                    ...
                    # Top 5% treasury within each LOWMOM bucket (treas_pct >= 0.95)
                    picks_list = []
                    for bucket in [0, 1, 2]:
                        bucket_stocks = this_tres[this_tres["mom_bucket"] == bucket]
                        if len(bucket_stocks) > 0:
                            top5 = bucket_stocks[bucket_stocks["treas_pct"] >= 0.95]
                            picks_list.extend(top5["ticker"].tolist())
                    picks = picks_list
                else:
                    picks = []
...
def main():
    ...
    strategies = ["LOWMOM60", "Treasury_Top20", "LOWMOM_Treasury_High5"]
    ...
    for strat_name in ["Treasury_Top20", "LOWMOM_Treasury_High5"]:
    ...
    for strat_name in ["LOWMOM_Treasury_High5"]:
    ...
    lowmom = results["LOWMOM60"]
    treasury = results["Treasury_Top20"]
    incremental = results["LOWMOM_Treasury_High5"]
    ...
    next_exp = "10-KR-23-24 STEP 3C: Test alternative cutoff"
    if judgment == "YES":
        next_exp = "10-KR-23-24 STEP 3C: Test cutoff sensitivity (Top5% selected)"
    elif judgment == "PARTIAL":
        next_exp = "10-KR-23-24 STEP 3C: Investigate cutoff sensitivity"
    else:
        next_exp = "10-KR-23-24 STEP 3C: Test alternative incremental construction"
    
    print(f"Next experiment suggestion: {next_exp}")

    # Save
    result = {
        "experiment": "10-KR-23-24 STEP 3B: TreasuryRatio Incremental Portfolio - Top5%",
    ...
    out_path = os.path.join(OUT_DIR, "kr-treasury-incremental-step3b-results.json")
