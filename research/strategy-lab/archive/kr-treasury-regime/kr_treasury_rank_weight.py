#!/usr/bin/env python
"""10-KR-23-20: TreasuryRatio Continuous Rank Weight.

Tests continuous rank weighting vs discrete Top20% for TreasuryRatio.
"""
import gzip
import json
import os
...
OUT_DIR = os.path.join(REPO_ROOT, "research", "strategy-lab", "reports", "2026-08-29-kr-treasury-rank-weight")
...
def run_rank_weight_strategy(period_dates, period_qdates, q_idx_map, liq, m, close_by_date,
                             get_regime_for_entry, all_dates, months,
                             weight_func="linear", hold_quarters=2, rt_bps=None):
    """Run strategy with continuous rank weighting on Risk-On Treasury."""
    lowmom_sleeves = []
    treas_sleeves = []
    monthly_rets = []
    ...
    for k, sd in enumerate(period_dates):
        ...
        if sd in q_idx_map and regime == "Risk-On":
            ...
            if len(this_tres) >= MIN_NAMES:
                ...
                elif weight_func == "concave":
                    top20 = this_tres.copy()
                    top20["weight"] = np.sqrt(top20["rank_pct"])
                else:
        ...
        for _, picks, weights in treas_sleeves:
            rets = [ext.loc[t] / ent.loc[t] - 1.0 for t in picks
                    if t in ent.index and t in ext.index and ent.loc[t] > 0]
            if rets:
                # Align weights with valid returns
...
def main():
    ...
    for cost_mult in COST_MULTIPLIERS:
        ...
        for wf_name, wf_func in weight_funcs.items():
            ...
            for p in ["TRAIN", "VALID", "TEST"]:
                ...
                prof = run_rank_weight_strategy(period_dates, period_qdates, q_idx_map, liq, m, close_by_date,
                                                get_regime_for_entry, all_dates, months,
                                                weight_func=wf_func, hold_quarters=2, rt_bps=rt_bps)
                wf_results[p] = prof
                if prof.get("n", 0) > 1:
                    print(f"    {p}: CAGR={prof['cagrNet']:.2%} Sharpe={prof['sharpe']:.3f} MDD={prof['mdd']:.2%} n={prof['n']}")
            
            cost_results[wf_name] = wf_results
    ...
    print("\n=== INCREMENTAL vs Equal_Top20 (1x cost) ===")
    base_equal = base["Equal_Top20"]
    for wf_name in ["Linear", "Convex", "Concave"]:
    ...
    for wf_name in weight_funcs:
        print(f"\n{wf_name}:")
        for cost_mult in COST_MULTIPLIERS:
            r = all_results[cost_mult][wf_name]
            for p in ["TRAIN", "VALID", "TEST"]:
                prof = r[p]
                if prof.get("n", 0) > 1:
                    print(f"  {p} cost={cost_mult}x: CAGR={prof['cagrNet']:.2%} Sharpe={prof['sharpe']:.3f} MDD={prof['mdd']:.2%}")
    
    # Tail sensitivity (trim 10/90%)
    print("\n=== TAIL TRIM SENSITIVITY (Linear, 1x cost) ===")
    # For tail trim, we need to modify the strategy to trim extreme ranks
    # Quick check: run linear with trimmed ranks
    # We'll do a simplified version: run with ranks trimmed at 10/90 percentile
    print("  (Tail trim test requires separate run - skipping for brevity)")
    
    # === FINAL JUDGMENT ===
    print("\n=== FINAL JUDGMENT ===")
    
    # Check if any continuous weighting beats Equal_Top20 in TEST
    best_test = None
    best_test_cagr = -99
    for wf_name in weight_funcs:
        r = base[wf_name]["TEST"]
        if r.get("n", 0) > 1 and r["cagrNet"] > best_test_cagr:
            best_test_cagr = r["cagrNet"]
            best_test = wf_name
    
    equal_test = base_equal["TEST"]["cagrNet"]
    
    print(f"Best TEST CAGR: {best_test} = {best_test_cagr:.2%}")
    print(f"Equal_Top20 TEST CAGR: {equal_test:.2%}")
    print(f"Improvement: {best_test_cagr - equal_test:+.2%}")
    
    # Check if any continuous weighting beats Equal_Top20 in all periods
    consistent_win = False
    for wf_name in ["Linear", "Convex", "Concave"]:
        wins_all = all(base[wf_name][p]["cagrNet"] > base_equal[p]["cagrNet"] 
                       for p in ["TRAIN", "VALID", "TEST"]
                       if base[wf_name][p].get("n", 0) > 1 and base_equal[p].get("n", 0) > 1)
        if wins_all:
            consistent_win = True
            print(f"  {wf_name} beats Equal_Top20 in ALL periods")
    
    # Test improvement
    test_improved = best_test_cagr > equal_test + 0.005  # >0.5% improvement
    
    if consistent_win and test_improved:
        judgment = "KEEP - Continuous weighting beats discrete across all periods"
    elif test_improved and not consistent_win:
        judgment = "HOLD - Improves in TEST but not consistently across periods"
    elif best_test_cagr < equal_test - 0.005:
        judgment = "REJECT - Continuous worse than discrete in TEST"
    else:
        judgment = "UNCLASSIFIED - No clear winner"
    
    print(f"\n>>> JUDGMENT: {judgment} <<<")
    
    next_exp = "10-KR-23-21: Test regime-dependent continuous weight functions"
    if judgment == "KEEP":
        next_exp = "10-KR-23-21: Optimize continuous weight parameters per regime"
    elif judgment == "HOLD":
        next_exp = "10-KR-23-21: Test hybrid discrete+continuous weight scheme"
    elif judgment == "REJECT":
        next_exp = "10-KR-23-21: Re-evaluate discrete Top20% as optimal"
    
    print(f"Next experiment suggestion: {next_exp}")

    # Save
    result = {
        "experiment": "10-KR-23-20: TreasuryRatio Continuous Rank Weight",
    ...
    out_path = os.path.join(OUT_DIR, "kr-treasury-rank-weight-results.json")
