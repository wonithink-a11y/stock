#!/usr/bin/env python
"""10-KR-23-10: TreasuryRatio 6M Hold + LOWMOM60 50/50 Combined.

Re-tests 10-KR-23-4 with Treasury sleeve at 6M hold (quarterly signal, 6M hold).
"""
import gzip
import json
import os
...
OUT_DIR = os.path.join(REPO_ROOT, "research", "strategy-lab", "reports", "2026-08-29-kr-treasury-lowmom60-6m")
...
def main():
    ...
    # === LOWMOM60 Baseline (Monthly, Top 30) ===
    ...
    # === TreasuryQuarterly 6M Hold (Top20%, quarterly signal, 6M hold) ===
    def run_treasury_6m(period_dates):
        out = []
        rebal_dates = [d for d in qmonths if d in period_dates]
        # 6M hold = 2 quarterly periods
        for k, sd in enumerate(rebal_dates):
            ...
            if ent_d is None:
                continue
            ext_d = rebal_dates[k + 2]  # 2 quarters = 6 months
            ent = close_by_date[ent_d]
            ext = close_by_date[ext_d]
            rets = [ext.loc[t] / ent.loc[t] - 1.0 for t in picks
                    if t in ent.index and t in ext.index and ent.loc[t] > 0]
    ...
    def run_combined(period_dates):
        # Monthly LOWMOM60 + Quarterly Treasury (6M hold)
        lowmom_sleeves = []
        treas_sleeves = []  # (quarter_idx, picks)
        monthly_rets = []
        ...
        for k, sd in enumerate(period_dates):
            # Add LOWMOM60 sleeve (monthly, 1M hold)
            ...
            # Add Treasury sleeve (quarterly, 6M hold)
            if sd in q_idx_map:
                qk = q_idx_map[sd]
            ...
            treas_sleeves = [(si, sp) for (si, sp) in treas_sleeves if current_q_idx - si < 2]  # 6M = 2 quarters
            ...
            monthly_rets.append({"ret": gr - ROUNDTRIP_BPS / 10000, "gross_ret": gr,
                                 "turnover": 0.5 * 1.0 + 0.5 * (1.0/6.0),
                                 "roundtrips": 0.5 * 1.0 + 0.5 * (1.0/6.0),
                                 "holding_months": 3.5,
                                 "picks": all_picks,
    ...
    results = {"LOWMOM60": {}, "Treasury6M": {}, "Combined_50_50": {}}
    overlap_data = {}
    ...
    for p in ["TRAIN", "VALID", "TEST"]:
        ...
        treas_rets = run_treasury_6m(period_dates)
        comb_rets = run_combined(period_dates)

        results["LOWMOM60"][p] = profile(lowmom_rets)
        results["Treasury6M"][p] = profile(treas_rets)
        ...
        overlap_data[p] = {"avg_monthly_overlap": round(avg_overlap, 3),
                           "return_corr": round(ret_corr, 3),
                           "unique_lowmom": len(lowmom_picks_all),
                           "unique_treasury": len(treas_picks_all)}

        print(f"  LOWMOM60:  CAGR={results['LOWMOM60'][p].get('cagrNet',0):.2%} Sharpe={results['LOWMOM60'][p].get('sharpe',0):.3f} n={results['LOWMOM60'][p].get('n',0)}")
        print(f"  Treasury6M: CAGR={results['Treasury6M'][p].get('cagrNet',0):.2%} Sharpe={results['Treasury6M'][p].get('sharpe',0):.3f} n={results['Treasury6M'][p].get('n',0)}")
        print(f"  Combined:  CAGR={results['Combined_50_50'][p].get('cagrNet',0):.2%} Sharpe={results['Combined_50_50'][p].get('sharpe',0):.3f} n={results['Combined_50_50'][p].get('n',0)}")
        print(f"  Overlap: {avg_overlap:.3f}, Return Corr: {ret_corr:.3f}")
    ...
    for p in ["TRAIN", "VALID", "TEST"]:
        ...
        inc = {
            ...
            "sharpeDiff": round((c.get("sharpe", 0) or 0) - (b.get("sharpe", 0) or 0), 4),
            "mddDiff": round(c.get("mdd", 0) - b.get("mdd", 0), 4),
            "grossCagrDiff": round((c.get("grossCAGR", 0) or 0) - (b.get("grossCAGR", 0) or 0), 4),
        ...
        print(f"  {p}: CAGR={inc['cagrDiff']:.2%} Sharpe={inc['sharpeDiff']:.3f} "
              f"MDD={inc['mddDiff']:.2%} GrossCAGR={inc['grossCagrDiff']:.2%} Turnover={inc['turnoverDiff']:.1f}x")
    ...
    if judgment == "PASS":
        next_exp = "10-KR-23-11: Test 6M Treasury + LOWMOM60 with regime timing"
    elif judgment == "WEAK":
        next_exp = "10-KR-23-11: Investigate why Treasury helps only in TEST"
    else:
        next_exp = "10-KR-23-11: Test TreasuryRatio with other baselines (PBR, REV20)"

    print(f"Next experiment suggestion: {next_exp}")

    result = {
        "experiment": "10-KR-23-10: TreasuryRatio 6M Hold + LOWMOM60 50/50 combined",
        "results": results,
    ...
    out_path = os.path.join(OUT_DIR, "kr-treasury-lowmom60-6m-results.json")
