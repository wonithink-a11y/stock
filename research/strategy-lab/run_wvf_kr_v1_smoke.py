#!/usr/bin/env python
"""WVF-KR-V1 smoke driver (STEP 2).

Mirrors run_macd_kr_v1_smoke.py / run_squeeze_kr_v1_smoke.py / run_supertrend_kr_v1_smoke.py structure exactly:
...
- State-machine entry/exit in build_events (dynamic mean-reversion exit NOT expressible in executor)
...
- Output: research/strategy-lab/findings/wvf-kr-v1-smoke/smoke_result.json
"""
...
OUT_DIR = REPO_ROOT / "research" / "strategy-lab" / "findings" / "wvf-kr-v1-smoke"
...
def run_subset(n_tickers: int, seed: int = 42):
    ...
    for i, sym in enumerate(subset):
        ...
        if len(bars) < WVF_LOOKBACK + BB_LEN + 5:
            continue
    ...
    result = {
        ...
        "indicators": {
            "wvf": {"lookback": WVF_LOOKBACK, "formula": "(Highest(Close,22)-Low)/Highest(Close,22)*100"},
            "bollinger": {"length": BB_LEN, "mult": BB_MULT, "source": "wvf"}
        },
    ...
    print("\nWVF-KR-V1 STEP 2 SMOKE driver ready.")
