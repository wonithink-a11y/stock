#!/usr/bin/env python
"""lowmom60_v1을 engine.runner.run_smoke()로 실제 실행 - 표준 A1A_ONLY라
pbr_value_v1과 같은 이유로 run_smoke()를 그대로 쓸 수 있다(파이프라인 복제
없음). run_smoke()는 진단만 반환하므로 CAGR/MDD/Sharpe는 이 스크립트가
engine/metrics로 계산한다.

  python run_lowmom60_v1.py
"""
import json
import os
import sys
...
def main():
    ...
    t0 = time.time()
    result = run_smoke("lowmom60_v1", START, END, repo_root)
    ...
    report = {
        "runIdentification": {
            "strategyId": "lowmom60_v1", "runClass": diag["runClass"],
        ...
        "comparisonToPrecheck": {
            "note": "lowmom60_institutional_eligible_precheck_v2_absolute.py "
                    "high_liquidity_absoluteThreshold (top30 하드컷, 동일가중 평균수익률 근사 - "
                    "이 실행은 engine의 실제 포트폴리오 회계(공유 현금·정수 주식수·같은날 현금 "
                    "재사용 금지 등)를 반영해 다를 수 있다)",
            "precheckCagr": 0.1390,
        },
    }

    out_dir = os.path.join(repo_root, "research", "strategy-lab", "reports", "2026-08-24-lowmom60-v1-smoke")
