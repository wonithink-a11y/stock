#!/usr/bin/env python
"""10-KR-23-24 STEP 4A: TreasuryRatio Incremental Portfolio Test - Top10%.

Tests 4 strategies with Treasury Top10% and High10% cutoffs:
1. LOWMOM60 baseline
2. Treasury Top20% baseline
3. Treasury Top10% standalone
4. LOWMOM 3-bucket × Treasury High10% (incremental)
"""
import gzip
import json
import os
...
def run_strategy(period_dates, period_qdates, q_idx_map, liq, m, close_by_date,
    ...
    for k, sd in enumerate(period_dates):
        ...
        # Strategy 3: Treasury Top10% only (decile: top 10%)
        elif strategy_name == "Treasury_Top10":
            w_treas = 1.0 if regime == "Risk-On" else 0.0
            w_lowmom = 1.0 - w_treas
        # Strategy 4: LOWMOM 3-bucket × Treasury High10%
        elif strategy_name == "LOWMOM_Treasury_High10":
            w_treas = 1.0 if regime == "Risk-On" else 0.0
            w_lowmom = 1.0 - w_treas
        else:
        ...
        if sd in q_idx_map and regime == "Risk-On":
            ...
            if len(this_tres) >= MIN_NAMES:
                if strategy_name == "Treasury_Top20":
                    ...
                    # Top 10% (Top decile)
                    this_tres = this_tres.copy()
                    this_tres["q"] = pd.qcut(this_tres["treasuryRatio"].rank(method="first"), 10, labels=False, duplicates="drop")
                    picks = this_tres[this_tres["q"] == 9]["ticker"].tolist()
                elif strategy_name == "LOWMOM_Treasury_High10":
                    # LOWMOM 3-bucket × Treasury High10%
                    this_tres = this_tres.copy()
                    ...
                    # Top 10% treasury within each LOWMOM bucket (treas_pct >= 0.9)
                    picks_list = []
                    for bucket in [0, 1, 2]:
                        bucket_stocks = this_tres[this_tres["mom_bucket"] == bucket]
                        if len(bucket_stocks) > 0:
                            top10 = bucket_stocks[bucket_stocks["treas_pct"] >= 0.9]
                            picks_list.extend(top10["ticker"].tolist())
                    picks = picks_list
                else:
                    picks = []
...
def main():
    ...
    strategies = ["LOWMOM60", "Treasury_Top20", "Treasury_Top10", "LOWMOM_Treasury_High10"]
    ...
    for strat_name in ["Treasury_Top20", "Treasury_Top10", "LOWMOM_Treasury_High10"]:
    ...
    for strat_name in ["Treasury_Top10", "LOWMOM_Treasury_High10"]:
...
# === FINAL JUDGMENT ===
    ...
    lowmom = results["LOWMOM60"]
    treasury_20 = results["Treasury_Top20"]
    treasury_10 = results["Treasury_Top10"]
    incremental = results["LOWMOM_Treasury_High10"]
    ...
    test_inc_vs_t10 = incremental["TEST"].get("cagrNet", -99) - treasury_10["TEST"].get("cagrNet", -99)
    test_inc_vs_t20 = incremental["TEST"].get("cagrNet", -99) - treasury_20["TEST"].get("cagrNet", -99)
    test_sharpe_inc_vs_lowmom = (incremental["TEST"].get("sharpe", 0) or 0) - (lowmom["TEST"].get("sharpe", 0) or 0)
    test_sharpe_inc_vs_t10 = (incremental["TEST"].get("sharpe", 0) or 0) - (treasury_10["TEST"].get("sharpe", 0) or 0)
    test_sharpe_inc_vs_t20 = (incremental["TEST"].get("sharpe", 0) or 0) - (treasury_20["TEST"].get("sharpe", 0) or 0)
    ...
    print(f"TEST vs Treasury_Top10: ΔCAGR={test_inc_vs_t10:+.2%} ΔSharpe={test_sharpe_inc_vs_t10:+.3f}")
    print(f"TEST vs Treasury_Top20: ΔCAGR={test_inc_vs_t20:+.2%} ΔSharpe={test_sharpe_inc_vs_t20:+.3f}")
    print(f"TRAIN vs LOWMOM60: ΔCAGR={train_inc_vs_lowmom:+.2%}")
    print(f"VALID vs LOWMOM60: ΔCAGR={valid_inc_vs_lowmom:+.2%}")
    
    # Judgment criteria
    test_beats_lowmom = test_inc_vs_lowmom > 0.002
    test_beats_t10 = test_inc_vs_t10 > 0.002
    test_beats_t20 = test_inc_vs_t20 > 0.002
    train_valid_consistent = (train_inc_vs_lowmom > -0.01 and valid_inc_vs_lowmom > -0.01)
    
    if test_beats_lowmom and test_beats_t10 and test_beats_t20 and train_valid_consistent:
        judgment = "YES"
    elif test_beats_lowmom or test_beats_t10 or test_beats_t20:
        judgment = "PARTIAL"
    else:
        judgment = "NO"
    ...
    next_exp = "10-KR-23-24 STEP 4B: Test cutoff sensitivity for Treasury Top10"
    if judgment == "YES":
        next_exp = "10-KR-23-24 STEP 4B: Treasury Top10 confirmed - test alternative constructions"
    elif judgment == "PARTIAL":
        next_exp = "10-KR-23-24 STEP 4B: Investigate Treasury Top10 sensitivity"
    else:
        next_exp = "10-KR-23-24 STEP 4B: Re-evaluate incremental construction"
    
    print(f"Next experiment suggestion: {next_exp}")

    # Save
    result = {
        "experiment": "10-KR-23-24 STEP 3A: TreasuryRatio Incremental Portfolio - Top10%",
    ...
    out_path = os.path.join(OUT_DIR, "kr-treasury-incremental-step3a-results.json")
