...
reason: >-
  횡단면 IC 는 3구간 전부 양(+)·부호일관·유의(TRAIN t=3.82 → VALID 4.24 →
  TEST 7.82, 오히려 강해짐)로 '사전정의 다개 quality 결합의 예측력'을
  지지한다. 그러나 실현 가능한 상위 decile 전략은 TEST(t=2.98, null
  p95=1.60 초과)에서만 난수선을 넘고 TRAIN(t=-0.45)은 오히려 음 — 'TRAIN
  강→TEST 붕괴'의 역방향으로 '최근 기간 전용' 신호다. 또 ROE 조건부 잔차
  IC 3구간 전부 ~0 이라 'composite 이 ROE 를 넘어서는 독립 정보'는
  아니다(상위 decile 페어 차 t=0.17~1.18 미유의). 재판정(2026-09-18, 새
  분석 아님 — 문서 본문 "왜 HOLD 인가" 표가 이미 적어둔 3가지를 그대로
  적용): 그 표 자체가 "연도집중도 51.4%(2024) → 40% 초과 → KEEP 아님"·
  "TRAIN 미달(-0.45)"·"ROE 대비 독립정보 미충족"을 스스로 명시했다 - 이
  세 게이트 미충족은 이 프로젝트가 다른 findings(PBR·EY 등)에서 같은
  패턴에 이미 REJECT/조건부 재분류를 적용해온 기준과 동일하다. "KEEP 도
  REJECT 도 단언 못한다"던 원 판정은 재론 여지를 남기려던 것으로
  읽히지만, 실행 후보 여부로 보면 이미 REJECT다.
cagr: 8.91
sharpe: 0.52
mdd: -21.8
win_rate: 58.1
...
## 왜 (원래) HOLD 였는가 — 2026-09-18 REJECT로 재판정, 아래 표는 원문 그대로 보존
...
결론(원문): **"사전정의 quality composite 의 횡단면 예측력" 자체는 관측으로 살아
...
**재판정(2026-09-18): REJECT.** 위 표의 세 항목(TRAIN 미달·연도집중도 40%
초과·ROE 독립성 미충족)이 이미 이 프로젝트가 다른 곳에서 같은 조합에
REJECT를 적용해온 것과 동일한 실패 모양이다 — "KEEP도 REJECT도 단언 못
한다"는 판단 보류였지 반박 근거가 아니었다. 실행 후보 여부로 보면 이
composite을 새로 production에 넣을 근거는 없다(REJECT). 재개하려면 IC가
아니라 "TRAIN에서도 통하는 실행 규칙"이나 "ROE를 실제로 능가하는 독립
정보"라는 새 증거가 필요하다 — 지금 있는 자료로는 안 나온다.
