...
const REQUIRED_UPSTREAM = {
  ...
  'A3b':  ['A1a', 'A1b', 'A3'],
  // A3d(공시 기반 기업행위 라벨, A5-3 D4)는 target_corps()로 A1a·A1b를 재사용하고,
  // multiplierSource.a3cBracket이 A3c 산출물을 직접 읽는다(새 API 호출 없이) — A3c가
  // 재수집되면 브래킷 배수가 달라질 수 있으므로 A3c를 상류로 선언한다(A3b가 A3를
  // 선언하는 것과 같은 이유).
  'A3d':  ['A1a', 'A1b', 'A3c'],
  'A4':   ['A0.5', 'A1a'],
  // A5는 resolver.js(A5-5)가 valuation D4 경로에서 A3b(배당/EPS)·A3c(발행주식수)·
  // A3d(기업행위 라벨)를 직접 읽는다 — A3만 선언하면 이 셋이 재수집돼도
  // verifyUpstream이 그 drift를 못 본다(A3b가 A3를 선언하는 것과 같은 이유).
  // A4(수급)는 2026-09-02까지 resolver.js에 연결이 안 돼 있어 A5 10년 백필의
  // supplyDemand 축이 100% 결측이었다(findings/kr-production-technical-
  // macross-reversal-2026-09.md 조사 중 발견) — supplyDemandFrom.js 신설로
  // 연결하며 여기 선언도 추가한다.
  'A5':   ['A0.5', 'A1a', 'A1b', 'A2a', 'A2b', 'A3', 'A3b', 'A3c', 'A3d', 'A4'],
  // EO(exit overlay)는 A2b의 exitAtConfirmed(가격 기반 실측 폐지일)와 A3d의
  // mergerSpinoff(Tier A, 합병 공시)를 시간축으로 조인한다 — 둘 중 하나라도
  // 재수집되면 분류가 달라질 수 있으므로 둘 다 상류로 선언한다. A1b는 대상
  // corp 집합(폐지 유니버스) 자체다.
  'EO':   ['A1b', 'A2b', 'A3d'],
  // A6은 A5(baked-in 사실)뿐 아니라 EO(승격된 최신 exitReason 분류)도 읽는다
  // (exit overlay가 있으면 우선, 없으면 A5 baked 값 — 위 문서 §1 결정).
  'A6':   ['A5', 'EO'],
  'A7':   ['A5'],
  // A8(공매도)은 A4와 같은 근거 - A0.5는 거래일 계약, A1a는 대상 유니버스.
  'A8':   ['A0.5', 'A1a'],
};
...
const REQUIRED_POLICIES = {
  ...
  // A5는 격자(universe)·raw 데이터(price·fundamentals)·자기 샤드/인수조건
  // (scores) 셋 다 필수다. score()가 실제로 읽는 criteria·confidence·
  // riskPenalty 등 scoringEngine 정책은 REQUIRED_POLICIES가 강제하지
  // 않아도(registry.policies 전량이라 이미 score() meta에 스탬프됨)
  // --policies에 항상 함께 얹는다(finalize 스텝).
  'A5': ['universe', 'price', 'fundamentals', 'scores'],
  // EO는 자기 분류 파라미터(windowDays·정규식 패턴) 하나만 필수다 — universe를
  // 안 요구하는 이유는 EO의 분모가 유니버스 정의가 아니라 A2b의 exitAtConfirmed
  // 존재 여부(가격 흔적)이기 때문이다.
  'EO': ['exitOverlay'],
  // A8(공매도)도 A4와 같은 이유로 universe를 요구한다.
  'A8': ['universe', 'shortSelling'],
};
