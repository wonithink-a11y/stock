...
function deepFreezeResult(obj) {
  if (process.env.NODE_ENV === 'production') return Object.freeze(obj);
  if (obj && typeof obj === 'object' && !Object.isFrozen(obj)) {
    Object.freeze(obj);
    for (const v of Object.values(obj)) deepFreezeResult(v);
  }
  return obj;
}
...
function collectFlags(confidence, riskPenalty, trading, components, axisResults, policies) {
  const known = new Set(Object.values(policies.flagCodes.groups).flat().map((f) => f.code));
  const flags = [];
