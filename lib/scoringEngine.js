...
function scoreStock(stockData, criteria) {
  if (!criteria) {
  criteria = require('./loadCriteria').loadCriteria('KR').criteria;   
  }
...
// ══════════════════ V2 (STEP5) — Pure Function 계층 ══════════════════
const { E, throwErr } = require('./errorCodes');

const ENGINE_VERSION = '2.1.1';
const SCHEMA_VERSION = '1.0.0';
const MATCH_FIELDS = new Set(['riskStates', 'listingStatus', 'tradingState']);

// §8 동결 축. 스코어러가 없다고 AX001로 죽이지 않되, 결과에 UNSUPPORTED_FEATURE를 남겨 조용히 넘어가지 않게 한다.
const unsupportedAxis = () => ({ score: null, detail: {}, coverage: { valid: 0, total: 0 }, unsupported: true });

const AXIS_SCORERS = {
  fundamental: scoreFundamental,
  valuation: scoreValuation,
  technical: scoreTechnical,
  supplyDemand: scoreSupplyDemand,
  institutionalFlow: unsupportedAxis,
  macro: unsupportedAxis,
};
...
/** 운영은 얕은 동결(비용)이되 meta·result는 반드시 얼린다 — 소비자가 잡는 참조가 봉투가 아니라 그 둘이다. */
function deepFreezeResult(obj) {
  if (process.env.NODE_ENV !== 'production') return freezeDeep(obj);
  Object.freeze(obj); Object.freeze(obj.meta); Object.freeze(obj.result);
  return obj;
}

// 정책은 immutable이라 flag 집합을 매 종목마다 다시 만들 이유가 없다.
const FLAG_SETS = new WeakMap();
function knownFlags(flagCodes) {
  let s = FLAG_SETS.get(flagCodes);
  if (!s) { s = new Set(Object.values(flagCodes.groups).flat().map((f) => f.code)); FLAG_SETS.set(flagCodes, s); }
  return s;
}

/** confidence 공식. validator(INV-007)가 같은 함수로 재계산해 검증하므로 공식은 여기 한 곳에만 둔다. */
function computeConfidenceValue(parts, cp) {
  const entries = Object.entries(cp.weights).filter(([k]) => k in parts);
  const usable = cp.ignoreNull ? entries.filter(([k]) => parts[k] !== null && parts[k] !== undefined) : entries;
  const tw = usable.reduce((s, [, w]) => s + w, 0);
  if (usable.length === 0 || tw === 0) return null;
  return round1(usable.reduce((s, [k, w]) => s + parts[k] * w, 0) / tw);
}

function computeConfidence(axisResults, cp) {
  let valid = 0, total = 0;
  for (const r of Object.values(axisResults)) {
    if (r && r.coverage) { valid += r.coverage.valid; total += r.coverage.total; }
  }
  const parts = { coverage: total > 0 ? round1((valid / total) * 100) : null, freshness: null, quality: null };
  return { value: computeConfidenceValue(parts, cp), ...parts };
}

// ⚠ RP-1.2 반영. 축(namespace)을 한 Set에 합치지 않는다.
function computeRiskPenalty(currentState, riskPolicy) {
  const st = currentState || {};              // state 파일 없는 종목 = 감점 0 (정상 경로)
  const matches = (rule) => {
    if (!MATCH_FIELDS.has(rule.matchField)) throwErr(E.RP001_UnknownMatchField, `${rule.code}:${rule.matchField}`);
    if (rule.matchField === 'riskStates') return Array.isArray(st.riskStates) && st.riskStates.includes(rule.matchState);
    return st[rule.matchField] === rule.matchState;
  };

  const byGroup = {}, adjustments = [];
  const emit = (r) => adjustments.push({ code: r.code, category: r.category, reason: r.reason, penalty: r.penalty });
  for (const r of riskPolicy.rules) {
    if (r.enabled === false) continue;        // 비활성 규칙은 matchField 검사도 하지 않는다(R101은 matchField:null)
    if (!matches(r)) continue;
    if (!r.exclusiveGroup) { emit(r); continue; }
    const g = byGroup[r.exclusiveGroup];
    if (!g || r.penalty < g.penalty) byGroup[r.exclusiveGroup] = r;   // 같은 그룹이면 더 큰 감점만
  }
  Object.values(byGroup).forEach(emit);
  return adjustments;
}

function evaluateTradingState(currentState, tp) {
  const state = (currentState && currentState.tradingState) || 'NORMAL';
  const blocked = tp.blockedStates.includes(state);
  return { tradeAllowed: !blocked, tradingState: state, tradingFlags: blocked ? ['TRADE_BLOCKED'] : [] };
}

/** components는 항상 양수라 방향 판단이 불가능하다. axis score의 50점 기준 편차로 산출한다. */
function computeTopFactors(axisResults, criteria) {
  const w = criteria.categoryWeights;
  const totalWeight = Object.values(w).reduce((s, x) => s + x, 0) || 1;
  return Object.entries(axisResults)
    .filter(([, r]) => r && r.score !== null && r.score !== undefined)
    .map(([axis, r]) => {
      const impact = round1(((r.score - 50) / 50) * (w[axis] / totalWeight) * 50);
      return { factor: axis, impact, direction: impact >= 0 ? 'positive' : 'negative' };
    })
    .sort((a, b) => Math.abs(b.impact) - Math.abs(a.impact));
}

function extractFeatures(axisResults) {
  const out = {};
  for (const r of Object.values(axisResults)) {
    for (const [key, d] of Object.entries((r && r.detail) || {})) {
      if (d && d.excluded === true) continue;
      out[key] = { raw: d.raw !== undefined ? d.raw : (d.peg !== undefined ? d.peg : null), score: d.score ?? null, source: d.source || null };
    }
  }
  return out;
}

/** scoreHistory가 쌓이기 전까지 null. 표본 없이 안정성을 지어내지 않는다. */
function computeStability(scoreHistory) {
  const xs = (scoreHistory || []).map((h) => (typeof h === 'number' ? h : h && h.finalScore)).filter((v) => typeof v === 'number');
  if (xs.length < 3) return null;
  const mean = xs.reduce((s, v) => s + v, 0) / xs.length;
  const sd = Math.sqrt(xs.reduce((s, v) => s + (v - mean) ** 2, 0) / xs.length);
  return { stability: round1(Math.max(0, 100 - sd * 5)), volatility: round1(sd) };
}

function collectFlags(confidence, riskPenalty, trading, components, axisResults, policies) {
  const known = knownFlags(policies.flagCodes);
  const flags = [];
  const push = (c) => { if (!known.has(c)) throwErr(E.FL001_UnknownFlag, c); flags.push(c); };

  const th = policies.confidence.thresholds || {};
  if (confidence.value !== null) {
    if (th.veryLowConfidence !== undefined && confidence.value < th.veryLowConfidence) push('VERY_LOW_CONFIDENCE');
    else if (th.lowConfidence !== undefined && confidence.value < th.lowConfidence) push('LOW_CONFIDENCE');
  }
  if (riskPenalty <= -25) push('VERY_HIGH_RISK');
  else if (riskPenalty <= -15) push('HIGH_RISK');

  if (Object.values(components).some((c) => c === null)) { push('MISSING_DATA'); push('PARTIAL_CALCULATION'); }
  if (Object.values(axisResults).some((r) => r && r.unsupported)) push('UNSUPPORTED_FEATURE');
  trading.tradingFlags.forEach(push);
  return flags;
}

/**
 * V2 엔진 진입점 — Pure Function. 실시간과 백테스트가 같은 함수를 쓴다.
 * @returns {{meta:object, result:object}} Immutable ScoreResult (§12)
 */
function score(stockData, criteria, policies) {
  const axisNames = Object.keys(criteria.categoryWeights || {});
  if (axisNames.length === 0) throwErr(E.AX002_EmptyCriteria);

  const axisResults = {}, items = [];
  for (const axis of axisNames) {
    const scorer = AXIS_SCORERS[axis];
    if (!scorer) throwErr(E.AX001_MissingAxis, axis);
    const r = scorer(stockData, criteria);
    axisResults[axis] = r;
    items.push({ axis, score: r.score, weight: criteria.categoryWeights[axis] });
  }

  const valid = items.filter((i) => i.score !== null && i.score !== undefined);
  const totalWeight = valid.reduce((s, i) => s + i.weight, 0);
  const renorm = policies.missingAxis.mode === 'renormalize';

  const components = {};
  let rawScore = null;
  if (valid.length > 0 && totalWeight > 0) {      // totalWeight 0 방어 — 없으면 NaN이 그대로 저장된다
    let sum = 0;
    for (const i of valid) {
      const c = (i.score * i.weight) / (renorm ? totalWeight : 1);
      components[i.axis] = round1(c);
      sum += c;
    }
    rawScore = round1(sum);
  }
  for (const a of axisNames) if (!(a in components)) components[a] = null;
  if (rawScore !== null && (rawScore < 0 || rawScore > 100)) throwErr(E.SC001_ScoreOutOfRange, `rawScore=${rawScore}`);

  const adjustments = computeRiskPenalty(stockData.state, policies.riskPenalty);
  const riskPenalty = adjustments.reduce((s, a) => s + a.penalty, 0);
  if (riskPenalty > 0) throwErr(E.PN001_PositivePenalty, riskPenalty);

  const finalScore = rawScore === null ? null : Math.max(0, Math.min(100, round1(rawScore + riskPenalty)));
  const trading = evaluateTradingState(stockData.state, policies.trading);
  const confidence = computeConfidence(axisResults, policies.confidence);

  return deepFreezeResult({
    meta: {
      schemaVersion: SCHEMA_VERSION,
      engineVersion: ENGINE_VERSION,
      market: policies.market || 'KR',
      calculatedAt: kstNow(),
      dataCutoff: stockData.dataCutoff || null,
      policies: { ...policies.versions },
    },
    result: {
      ticker: stockData.ticker || null,
      name: stockData.name || null,
      rawScore, riskPenalty, finalScore,
      ...trading,
      confidence,
      stability: computeStability(stockData.scoreHistory),
      flags: collectFlags(confidence, riskPenalty, trading, components, axisResults, policies),
      components,
      adjustments,
      topFactors: computeTopFactors(axisResults, criteria),
      features: extractFeatures(axisResults),
    },
  });
}
module.exports = {
  scoreStock, normalizeByThreshold, weightedAverage,          // V1 — 대시보드 계약 유지
  score, computeRiskPenalty, evaluateTradingState,            // V2
  computeConfidence, computeConfidenceValue, computeTopFactors,
