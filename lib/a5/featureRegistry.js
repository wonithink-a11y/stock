...
const FEATURES = {
  ...
  // 2026-08-12 available:true로 전환 — lib/a5/resolver.js의 shareholderReturnFrom()이
  // A3b dividendPerShare 5년 이력에서 실제로 유도한다(구현·회귀 확인,
  // docs/verification/BF-1.1-수직슬라이스-결과.md). alotMatter는 이미 수집 완료됐다.
  shareholderReturn:    { category: 'fundamental', stage: 'A3b', available: true,
                          needs: ['dividendPerShare'] },

  // ── valuation ──────────────────────────────────────────────
  // 2026-08-12 blockedBy 정정 — EPS는 이미 A3b에 있다(더 이상 미수집이 아니다).
  // 실제 블로커는 조정 기준 불일치다: A2a 가격은 adjusted:true(수정주가)인데 A3b
  // EPS는 DART 원문 그대로(조정 없음)라 price/eps를 그대로 나누면 액면분할을 겪은
  // 종목에서 틀린 값이 나온다(005930/2016-04-08 실측, per=0.197). perRelative는
  // 횡단면 계산이 추가로 더 필요해 available:false를 유지한다.
  perRelative:          { category: 'valuation', stage: 'A3b', available: false,
                          needs: ['eps', 'close', 'sicCode'], crossSectional: true,
                          blockedBy: 'A2a(adjusted 수정주가) vs A3b EPS(원본) 조정 기준 불일치 + 횡단면(peer) 인프라 미구축',
                          note: '업종 중앙값 대비. 평균이 아니다 — 적자 종목의 PER이 튄다' },
  // stage가 A3b가 아닌 이유: A3b의 범위가 alotMatter(EPS·배당)로 확정됐고 발행주식총수는
  // 거기서 안 나온다(A3b-결정브리프 §6). stage에 A3b를 두면 weightUnlockedBy가 pbr까지
  // 켜서 A3b의 효과를 0.68 대신 0.74로 과대 추정한다 — 실제로 A5-1.0 §5가 그 값을 적었다.
  // 2026-08-20 blockedBy 갱신 — A5-3 D4(공시 라벨 기반 소급 보정) 방향 확정 후
  // lib/a5/corporateActionAdjustment.js + resolver.js의 sharesOutstandingFrom·
  // valuationD4From을 구현하고 scripts/test-a5-d4.js(N=13 실사례 + 경계값)로 회귀
  // 통과 확인(docs/A5-3-peg-조정기준-결정브리프.md §17). **함수는 됐지만 여전히
  // available:false다** — corporateActions(공시 라벨)를 실제로 채워줄 백필 단계가
  // 아직 없다. featureRegistry의 available은 "지금 그 단계가 실제로 값을 주는가"의
  // 실측이지 스펙이 아니다(이 파일 상단 원칙) — 코드가 맞아도 상류가 없으면 false다.
  pbr:                  { category: 'valuation', stage: 'A3d', available: false,
                          needs: ['equity', 'sharesOutstanding', 'corporateActions', 'close'],
                          blockedBy: '함수·회귀는 구현됨(2026-08-20). 공시 라벨링 백필 단계(A3d, 가칭)가 아직 없어 corporateActions를 실제로 공급할 상류가 없다' },
  // 2026-08-20 stage·needs 정정 — a3c 블록의 EPS 우회식(netIncome÷sharesOutstanding)을
  // 따라 A3b EPS를 더 이상 쓰지 않는다(그쪽은 조정 기준 불일치가 여전히 안 풀렸다).
  // peg도 pbr과 같은 상류(A3+A3c+A3d corporateActions)로 갈아탔다.
  peg:                  { category: 'valuation', stage: 'A3d', available: false,
                          needs: ['netIncome', 'sharesOutstanding', 'corporateActions', 'close'], history: 2,
                          blockedBy: '함수·회귀는 구현됨(2026-08-20, netIncome÷sharesOutstanding 우회식으로 A3b 조정 기준 문제를 피해 간다). 공시 라벨링 백필 단계(A3d, 가칭)가 아직 없어 corporateActions를 실제로 공급할 상류가 없다' },
  ...
  // 2026-08-12 lib/a5/technicalFrom.js로 구현 확인(scripts/collect.js 계산식
  // 재사용). available:true는 "계속 상장 종목"에 한정된다 — A2b(폐지 종목 가격)가
  // 아직 0건 수집이라(A5-2, T1 뒤) 전체 유니버스로는 아직 아니다.
