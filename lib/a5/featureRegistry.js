...
const FEATURES = {
  ...
  pbr:                  { category: 'valuation', stage: 'A3c', available: false,
                          needs: ['equity', 'sharesOutstanding', 'close'],
                          blockedBy: '발행주식총수가 파이프라인 어디에도 없다' },
  // 2026-08-12 blockedBy 정정 — "PER 기반이라 EPS를 따라간다"는 EPS 미수집 시절의
  // 문구다. 지금 실제 블로커는 perRelative와 같다(조정 기준 불일치). lib/a5/resolver.js의
  // valuationFrom()이 공식은 구현했지만 resolve()가 호출하지 않는다.
  peg:                  { category: 'valuation', stage: 'A3b', available: false,
                          needs: ['eps', 'close'], history: 2,
                          blockedBy: 'A2a(adjusted 수정주가) vs A3b EPS(원본) 조정 기준 불일치' },
