...
const FEATURES = {
  ...
  // 통과 확인(docs/A5-3-peg-조정기준-결정브리프.md §17).
  // 2026-08-21 available:true로 전환 — A3d(공시 라벨링 백필) 전체 유니버스
  // 실제 재수집·finalize 완료(manifest 커밋 ca9cffd, acceptancePassed:true).
  // 같은 날 발견한 브래킷 배수 버그 2건(PIT 정렬순서·방향 인식)도 고쳐 재수집에
  // 반영했다 — 실제 산출물에서 방향모순(split<1·reverseOrConsolidation>1) 0건
  // 직접 재확인(research/strategy-lab/findings/a3d-bracket-candidates/README.md).
  // corporateActions를 공급할 상류가 이제 있다.
  pbr:                  { category: 'valuation', stage: 'A3d', available: true,
  ...
  // 2026-08-21 available:true로 전환 — 위 pbr과 같은 이유(A3d 재수집 완료).
  peg:                  { category: 'valuation', stage: 'A3d', available: true,
