...
const { supplyDemandFrom } = require('./supplyDemandFrom');
...
function resolve({ ticker, corp, asOf, fundamentals = [], dividendEps, candles,
                   ...
                   sicCode = null, quality = null, supplyDemandRecords }) {
  ...
  const hasSupplyDemand = supplyDemandRecords !== undefined;
  ...
  const val = hasSharesOutstanding
    ...
    : null;
  const sd = hasSupplyDemand ? supplyDemandFrom(supplyDemandRecords, asOf) : null;
  ...
  if (hasSupplyDemand) {
    for (const [k, v] of Object.entries(sd.values)) {
      if (v === null || v === undefined) missing.push(k);
    }
  }
  ...
  return {
    ...
    provenance: {
      ...
      ...(hasSupplyDemand ? { supplyDemand: { windowSize: sd.windowSize, lastDate: sd.lastDate } } : {}),
