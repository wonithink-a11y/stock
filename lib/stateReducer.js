...
function reduce(prevState, event, stateMapPolicy) {
  ...
  if (!event.sortKey) throw new Error('Invalid event.sortKey');

  // REDUCE-006: 이미 반영된 시점보다 이르거나 같은 이벤트는 무시(재실행 멱등 + 역순 방어)
...
module.exports = { reduce, initState, validateState, validateEvent, validatePolicy, STATE_SCHEMA_VERSION };
