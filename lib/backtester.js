 ...
 */
function classifyBase(s, model) {
  if (s.score === null || s.score === undefined) return EXCLUSION.NOT_SCORED;
  if (model && !axisBasis.matchesModel(s.basis || [], model)) return EXCLUSION.BASIS_MISMATCH;
...
function runBacktest(snapshots, criteria, options = {}) {
  ...
  for (const s of scored) {
    const why = classifyBase(s, model);
  ...
  const population = {
    ...
    observedBasis: countBy(scored.map((s) => axisBasis.basisKey(s.basis || []))),
  ...
  } else if (!population.model.matchesOperationalModel) {
    const ex = Object.keys(population.model.excludedAxes).join(' · ') || '(없음)';
    verdicts.push(
      `이 결과는 ${population.model.axisModel} 모델의 백테스트입니다 — 운영 모델이 아닙니다. ` +
      `빠진 축: ${ex}. ${population.model.promotionRule || ''}`.trim()
    );
  }
