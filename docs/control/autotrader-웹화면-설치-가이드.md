...
- 로그인은 **비밀번호 + 인증앱 숫자 한 번**입니다. 로그인하면 요약과 상세(보유·잔고)를 볼 수 있어요.
- **30분 동안 아무것도 안 하면** 자동으로 로그아웃됩니다. 5번 틀리면 15분 동안 잠깁니다.
- 로그인을 시도할 때마다 **텔레그램으로 알림**이 옵니다(성공·실패 모두). 내가 안 한 로그인이면 바로 알 수 있어요. (아래 8번)
- 더 엄격하게 하고 싶으면(상세를 볼 때마다 숫자를 다시 묻게) 설정 파일에서 켤 수 있어요. (아래 9번)
...
- [ ] 요약 화면에 **종목 이름·금액이 없다**(정상! 종목·금액은 상세 화면에서만 나와요).
- [ ] 아래 "보유·주문 상세 보기"를 누르면 **바로** 보유·잔고가 나온다.
- [ ] 30분 동안 아무것도 안 하다가 누르면 **다시 로그인 화면**이 나온다.
...
## 8. 텔레그램으로 로그인 알림 받기 (누가 로그인을 시도하면 바로 폰에 알림)

로그인에 **성공하면 ✅ 알림**, **실패하면 ⚠️ 알림**이 옵니다(실패가 많으면 한 통으로 묶어 보내요). 내가 안 한 성공 알림이 오면 바로 비밀번호를 바꿉니다.

> 알림을 보내는 프로그램은 **화면 프로그램과 따로** 돌아요. 텔레그램 열쇠(토큰)는 알림 프로그램만 가지고 있어서, 화면이 뚫려도 토큰은 안전합니다.

### 8-1) 준비물 세 가지 (텔레그램 앱에서)
이미 쓰고 계신 알림 봇(`@wonistock_bot`)을 그대로 씁니다.
...
### 8-2) VM에 두 줄 적기 (VM 안에서)
```bash
nano ~/collector-venv/.env
```
- 파일이 열립니다. **맨 아래 빈 줄**로 가서 아래 두 줄을 추가하세요. (`여기에…` 자리를 진짜 값으로 바꾸세요. **기존 줄은 하나도 지우지 마세요.**)
```
...
```
```bash
AUTOTRADER_ENV_FILE=~/collector-venv/.env ~/collector-venv/bin/python3 -m autotrader --config ~/collector-venv/autotrader/autotrader.local.json notify-logins --test
```
- `테스트 메시지를 보냈다` 라고 나오고, **폰 텔레그램에 "🔔 autotrader 텔레그램 알림 테스트"** 가 오면 성공이에요.
- `환경변수 TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID 가 없다` 라고 나오면 8-2를 다시 확인하세요(오타·빠뜨림).
- `전송 실패` 라고 나오면 8-1의 3번(봇에게 `/start`)을 했는지, 토큰을 정확히 옮겼는지 확인하세요.

### 8-4) 알림 프로그램 켜기 (VM 안에서)
```bash
sudo cp deploy/autotrader-notify.service /etc/systemd/system/
```
```bash
sudo systemctl daemon-reload
```
```bash
sudo systemctl enable --now autotrader-notify.service
```
```bash
sudo systemctl status autotrader-notify --no-pager | head -8
```
`active (running)` 이 보이면 켜진 거예요.

### 8-5) 진짜로 되는지 확인
1. 화면 프로그램에도 새 코드를 적용합니다:
```bash
sudo systemctl restart autotrader-web
```
2. 폰에서 로그인 화면으로 가서 **일부러 틀린 비밀번호**로 한 번 로그인해 보세요 → 몇 초 안에 ⚠️ 실패 알림이 와야 해요.
3. 이번엔 **맞게** 로그인하세요 → ✅ 성공 알림이 와야 해요.

알림이 안 오면 `~/collector-venv/logs/autotrader-notify.log` 를 캡처해서 저에게 보내주세요. (토큰은 로그에 안 남게 만들었어요.)

---

## 9. 설정으로 바꿀 수 있는 것 (원하는 사람만)

설정 파일 `~/collector-venv/autotrader/autotrader.local.json` 에 `"web"` 항목이 있으면 그 값이 쓰이고, 없으면 기본값이에요. **손대지 않아도 잘 돼요.**

| 항목 | 기본값 | 뜻 |
|---|---|---|
| `idle_min` | 30 | 이만큼(분) 아무것도 안 하면 로그아웃 |
| `session_hours` | 8 | 계속 써도 이 시간(시간)이 지나면 다시 로그인 |
| `require_reauth_for_details` | false | `true` 로 바꾸면 상세를 볼 때마다 인증앱 숫자를 **한 번 더** 물어요(더 안전, 더 불편) |
| `reauth_min` | 5 | 위 `true` 일 때, 다시 물어보기 전까지 상세가 열려 있는 시간(분) |

바꾼 뒤에는 `sudo systemctl restart autotrader-web` 한 줄이면 적용돼요.
