...
| LAB-1 | 조기 종료가 목표 집단을 삼켰는가 | Claude★ | Codex(대기) | `DONE(잠정)` ★ 16개 현재상장 갇힘, 재검증으로 13개(신규상장)+2개(신탁업,불확실)+1개(기존확인)로 세분 | — | `docs/verification/LAB-1-조기종료-결과.md` |
...
LAB-1  결과(Claude 잠정, 2026-08-12) — 조기 종료(EARLY_STOP) 754개 회사 중
       ...
       확인한 사례. 재수집 대상은 13개로 좁혀 잡는다. 013이 2015~2017에
       몰리는 건 제도 차이가 아니라 스캔 순서(오래된 해부터) + 조기종료
       규칙의 기계적 결과
