...
```
  완료      ★★ 모의투자 월간 자동화 5개 링크 완성 + 실패 알림 (2026-09-09~10) —
            A2a(월 1~5일, 중복 게이트) → refresh-selections(workflow_run) →
            VM collector-pull(06:30, --ff-only 읽기전용) → 페이퍼 엔진(10분).
            refresh-selections 는 CI 8회차에 완주(run 34331601465, 16분 30초).
            7번의 실패가 잡은 것: 의존성 3종(pytest·pyarrow·scipy — pyarrow 는
            import 문에 없어 정적 스캔 불가) · gitignore 대상이라 러너에만 없는
            데이터(reports/·data/a4/) · node 빌더의 박힌 종료일. 실패 중 커밋 0건.
            경위·함정 전문은 세션인수인계-2026-09-09-b.md.
            ★ 알림 채널이 애초에 없었다 — gh secret list 에 TELEGRAM_* 가 없어
            scripts/notify.js 가 매일 "(알림 채널 미설정)"만 찍고 종료하고
            있었다(continue-on-error 라 초록불로 보였다). 2026-09-09 봇
            @wonistock_bot 생성 + 시크릿 등록 + 실제 발송 확인.
            .github/workflows/notify-failure.yml — 워크플로마다 if: failure()
            를 붙이지 않고 workflow_run 한 곳에서 받는다(대상이 늘 때 N군데를
            고치지 않기 위해). 대상 4개, **'성공→실패' 전이에만** 보낸다 —
            페이퍼 엔진이 10분마다 돌아 매번 보내면 하루 144통이 되고 그러면
            채널을 끄게 된다(알림이 있는 것보다 나쁜 상태).
            ☆ 남은 실물 검증은 2026-10-01 — workflow_run 물림이 실제로 도는지는
            그날 처음 드러난다(지금까지 workflow_dispatch 로만 태웠다).

  완료      인수인계 2026-09-09-b "남은 것" 중 넷은 2026-09-10 실측으로 닫혔다
            — 셋은 이미 해소돼 있었고 하나는 기록이 틀렸다.
              부분체결 잔여 5건    ENTRY_SUBMITTED 0건, 137 전부 OPEN. 다 체결됐다
              매매불가 무한재시도   09-09 의 132건은 VM pull(10:49) 이전 것. 가드는
                                   paperEngine.py:285·447 양쪽에 다 있다
              foreign_flow5d 정체   결함 아님. 장마감 후 생성돼 전부 40580000
                                   (장종료). 다음 영업일 09:00 회차가 제출한다
              run_pbr_value_v1     "미수정"이라 적혀 있었으나 이미 격리돼 있다
                                   (deprecatedRealizedPnL, "인용 금지", 정본은 MTM)
            ★ 넷 중 셋이 문서가 실물보다 낡은 경우였다 — 인수인계의 "남은 것"을
            그대로 착수 목록으로 삼지 말고 실측부터 한다.

  완료      run_capacity_test.py 의 라이브 policy.json 바꿔치기 제거 (2026-09-10,
            2b5ff6a) — policy.json 을 policy_100.json 으로 덮어쓰고 finally 에서
            되돌리고 있었다. finally 는 크래시만 막는다. 진짜 문제는 **경합**이다
            — 페이퍼 엔진이 10분마다 같은 파일을 읽는데(rule.py 가 import 시점에
            로드) 이 스크립트는 조합 하나에 수십 분을 쓴다. run_smoke 가 이미
            rule_module 을 받으므로 load_strategy 후 PARAMS 한 줄 대입으로 바꿨다
            (load_strategy 는 호출마다 exec_module 이라 격리된다).
            ★ policy_20/50/100.json 은 지웠지만 **policy_30.json 은 남긴다** —
            market_comparison_kr_2026_09.py 가 동결 기준본으로 json.load 한다.
            처음에 4개를 다 지웠다가 grep 해서 되살렸다(자르기 전에 호출자를
            전부 훑는다). tests/test_capacity_test_policy_isolation.py 가 pin 한다.

  완료      페이퍼 폴 타이머를 장 마감(15:30)에서 끊었다 (2026-09-10) —
            9..15:00/10:00 은 15:50 까지 돌았다. 옛 주석은 "장마감 이후 회차는
            같은 결과를 다시 확인하는 것뿐이라 해롭지 않다"고 했는데 실측과
            다르다: 남은 PENDING_ENTRY 를 회차마다 재제출하고 40580000 으로
            거부당한다(하루 60건). 엔진에는 장중 판정이 아예 없으므로
            (engine/ 전체 grep) 그 경계를 정하는 것은 타이머뿐이다.
            09..14:00/10:00 + 15:00,10,20,30:00 으로 나눠 40회.
...
## 추가 분리 (2026-09-06, 같은 세션) — "다음"·"착수 가능"·"언제든" 중 완결된 항목

CLAUDE.md 상태블록을 "완료"뿐 아니라 "다음"·"착수 가능"·"언제든"에서도 점검한
결과, 라벨과 달리 이미 결론이 난 항목이 다수 발견됐다(사용자 지시로 확인 후
분리). 아래는 원래 그 라벨 아래 있던 전체 서술이다 — 요약·재작성 없음.

### (원래 "현재 트랙", 완전히 과거형이라 CLAUDE.md에서 삭제)
...
### (원래 "다음" — PBR 연구 라인 종료, CLAUDE.md엔 압축 요약만 남김)
...
### (원래 "언제든" — BF-1.1/GATE-EP-1 + 2026-09-02 후속 1~4, 대부분 최종 결론 남)
