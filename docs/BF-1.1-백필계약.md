...
3. `|ROE| > 200%` 건수 리포트
4. 조인 키는 **`corp_code`**
5. DART 일 한도 40,000건(FN-1.7, 2026-08-13 사용자 확인 — 옛 20,000은 근거 문서를 못 찾은 관측값이었다). 실패 시 다음날까지 대기이므로 **resume 로직 필수**
...
**`failureClassification`은 계약 해시에 넣지 않는다 — 한 번 넣었다가 뺐다.**

이 왕복이 판정 기준을 가장 정확하게 설명하므로 결론만 적지 않고 남긴다.

넣었던 근거는 *"어떤 status를 '재시도 불가'로 옮기면 그 법인은 계속 재시도되는 대신 승인
대상 공백이 된다"* 였다. 그럴듯하지만 **이 구현에서는 사실이 아니다.** 코드에서 `retryable`은
세 곳에만 나타난다 — `hardSkipped` 항목의 라벨, `hardSkippedByRetryable` 진단 집계, 중단
메시지의 힌트. `todo` 계산에도 `shard_status`에도 완료 게이트에도 들어가지 않는다.
`todo`는 '`done`에 없는 전부'이므로 **재시도 불가로 분류된 법인도 매 실행 똑같이 재시도되고**,
공백을 닫는 것은 오직 사람의 승인이다. 게다가 그 라벨은 다음 재시도에서 현재 표로 다시
계산되므로 **낡음이 자가 치유된다.**

그래서 넣으면 비용만 남는다. 오류 원인 분해 과정에서 새 status를 발견해 표에 넣는 것은
정상적인 개선인데, 그때마다 며칠치 수집을 잃는다.

```
판정 기준은 '결과에 영향을 주는가'가 아니라
            '이미 모은 것을 다시 쓸 수 있는가'다.
```

두 질문은 대개 같은 답을 주지만 여기서 갈렸다. 회귀가 이 근거를 직접 검증한다 — 분류표만
다르게 해서 두 번 수집하고, `retryable` 라벨을 뺀 상태가 완전히 같은지와 완료 판정이 같은지를
본다. 만약 나중에 `retryable`이 실제로 수집 경로를 가르도록 바뀌면(예: 재시도 불가는 `todo`에서
제외) 그 회귀가 먼저 깨지고, 그때는 이 표가 `collectionContract.fields`로 들어와야 한다 —
**그 변경과 이 목록은 함께 움직인다.**

**계약의 범위는 개수가 아니라 목록이 정한다.** "필드 몇 개"로 적어두면 몇 달 뒤 누군가
`stopAfterConsecutiveEmptyYears`를 빼거나 `requestSleepSeconds`를 넣어도 아무도 모른다.
`test-policies.js`가 목록 전체를 리터럴로 못 박고, `test-fundamentals-a3.py`가 동일/변경을
표로 검증한다(운영값 12건 → 해시 동일, 계약값 11건 → 해시 변경). 범위를 바꾸려면 그
단언을 함께 고쳐야 하고, 그 diff가 근거로 남는다.

**manifest에는 계약 해시를 기록하되 대조하지는 않는다.** finalize가 `_shards/`를 지우므로
manifest의 `collectionContractHash`·`collectionPolicyVersions`가 "어떤 계약으로 모았는가"의
유일한 생존 기록이다. 다만 그 값이 상태에서 나온 것이므로 *"manifest == 상태"* 를 검사하는
것은 구성상 항상 참인 동어반복이고, 아무것도 잡지 못한다(교훈45). 샤드 간 계약 불일치는
finalize가 별도로 중단시킨다 — 검사할 자리와 기록할 자리가 다르다.
...
#### 승인 채널 — 규칙과 예외를 manifest에서 갈라 놓는다 (REG-1.5)

`retryable == false`는 재시도로 풀리지 않으므로 완료 판정이 영구히 막힌다. 탈출 경로는
사람의 승인 하나뿐이고, 그 승인이 **어디에 기록되는가**가 이 절의 내용이다.

```
config/policies/fundamentals.v1.json    failureClassification — 규칙 (수집 계약 해시에 포함)
config/backfill/declared-gaps-a3.json   승인 목록 — 예외 (approvalHash로 별도 기록)
```

같은 파일에 두면 corp 하나를 승인할 때마다 `policyHash`가 바뀌어 **그 정책을 읽는 모든
단계의 manifest에 영향**이 간다. 바뀐 것은 규칙이 아니라 예외 목록인데도 그렇다. 그래서
registry에 네 번째 네임스페이스(`approvals`)를 만들고 manifest에 `approvalHash` 필드를
따로 둔다 — 정책 변경과 운영 승인이 **서로 다른 이벤트**로 갈린다.

`--extra`에 해시를 얹지 않는다. 그러면 **선언이 강제되지 않고**, 이 저장소는 이미 같은
이유로 그 형태를 거부했다("선언 자체의 누락은 REQUIRED 표가 잡는다"). `REQUIRED_APPROVALS`가
`A3 → declaredGapsA3`를 요구하며, 누락되면 `writeManifest`가 산출물을 해싱하기도 전에 던진다.

**승인은 수집 동작을 바꾸지 않는다.** 승인된 법인도 다음 실행에서 똑같이 재시도되고,
승인이 하는 일은 완료 판정에서 그 공백을 '열린 것'으로 세지 않는 것뿐이다. 이 분리가
`approvalHash`를 `collectionContractHash`와 따로 두는 근거다 — **승인이 수집 결과를 바꾸면
그것은 승인이 아니라 규칙이고, 규칙이라면 수집 계약 해시에 들어가야 한다.** 회귀 테스트가
이 성질을 직접 검증한다(승인된 법인이 다음 실행에서 `attempts`를 올리는지 본다).

승인 항목은 `{corp, reason}` 둘뿐이다. `decidedBy`·`approvedInCommit`을 넣지 않는 이유는
git이 이미 작성자·시각·이력을 보존하고, **항목을 추가하는 그 커밋의 SHA는 그 시점에 알 수
없기** 때문이다. `approvalHash`가 "어느 버전의 승인 목록으로 만들어졌는가"를 결정론적으로
고정하고, 누가·언제는 `git log --follow`가 답한다.

빈 배열도 해시한다. **'승인이 없다'와 '승인 파일이 없다'는 다르다** — 후자는 채널이
배선되지 않았다는 뜻이고, 그것을 빈 목록으로 조용히 읽으면 승인 체계가 있는 척만 하게 된다.
파일 부재는 예외이고, `reason`이 빈 승인도 거부한다(재검토할 근거를 남기지 않는다).

실제 공백에 대응하지 않는 승인(`declaredNotInHardSkipped`)은 **WARN**이다. FAIL이 아닌
이유는 정당한 경우가 있어서다 — 승인한 법인이 나중에 성공하면 `hardSkipped`에서 빠지고
승인만 남는다. 다만 그대로 두면 **오래된 승인이 미래의 공백을 미리 덮고**, 그것이 이 체계가
조용해지는 유일한 경로다.

#### 정정 — timeout은 처음부터 있었다 (2026-08-05)

collect #1 장애 분석은 `dart_call`·`fetch_sic`의 `requests.get`에 `timeout=` 인자가 없다는
것을 근거로 "timeout 부재"를 결함으로 올렸고, 재실행 지연(1.55초/호출)과 스텝 소요를
그 위에서 해석했다. **틀렸다.** `build-fundamentals-a3.py`는 최초 커밋(`30b570f`)부터
`HTTPAdapter.send`를 몽키패치해 인자가 없는 모든 요청에 `timeout=(10, 60)`을 넣고 있었다.
호출부에 인자가 없는 것이 timeout이 없다는 뜻이 아니었다.

```
관측했던 것   호출부 두 곳에 timeout= 인자가 없다
결론했던 것   timeout이 없다 → 무한 대기 가능 → 커밋 3에서 추가한다
실제          전역 어댑터 기본값 (10, 60)이 이미 적용 중이다
```

바뀌는 것은 항목 하나가 아니라 **설계 근거 자체**다. 남는 논의는 '추가할 것인가'가 아니라
`(10, 60)`이 적정한가 하나다 — 실측 최댓값 1.55초/호출의 약 39배 여유이고, 정찰 스크립트가
쓰는 `(10, 40)`과 갈린다. 지연 편차가 경합이 아니라 DART 쪽 변동에서 왔으므로(재실행의
샤드 1은 혼자 돌았는데도 1.55초/호출) 빡빡하게 잡을 이유는 없다. **값 조정은 실측이 더
쌓인 뒤에 하고, 그때까지 `(10, 60)`을 유지한다.**

교훈 41의 다른 얼굴이다 — 그때는 증상이 원인을 지목하지 않았고, 여기서는 **부재의 증거가
부재가 아니었다.** 호출부만 보고 전역 설정을 확인하지 않았다.
...
3,801법인 × 11사업연도 = 41,811건이므로 일 한도 40,000건(FN-1.7)에도 여전히 안 들어온다.
