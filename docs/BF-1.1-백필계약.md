...
```
Validated against
  정책        UN-1.2 · REG-1.3 · EP-1.0
  구현        91b9455   ← 이 문서가 검증된 마지막 구현 커밋
  검증 범위   A0.5 · A0.7 · A1a · A1b (A2 이후는 설계이며 아직 구현과 대조된 적이 없다)
...
```bash
git log --oneline 91b9455..HEAD -- lib scripts config .github
```
...
4. **`_retry(..., allow_empty=False)` 필수** — 빈 결과를 성공으로 기록하지 않는다
5. 연도별 matrix 분할(Actions 6시간 제한). 각 30분 이내
6. **대량 루프 진입 전 정찰 2회 + 연속 실패 서킷 브레이커** (교훈32)
