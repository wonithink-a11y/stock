...
```
Validated against
  정책        UN-1.2 · PR-1.0 · REG-1.3 · EP-1.0
  구현        8f041c9   ← 이 문서가 검증된 마지막 구현 커밋
  검증 범위   A0.5 · A0.7 · A1a · A1b (실행 완료) · A2a (구현 완료, 첫 수집 대기)
...
```bash
git log --oneline 8f041c9..HEAD -- lib scripts config .github
```
...
거래일 누락률(전체 1% / 종목별 10%)과 정상 공백 비율은 WARN이다. **거래정지 구간에 KRX가 행을 주는지 안 주는지를 아직 실측한 적이 없다.** FAIL로 걸면 정당한 데이터가 파이프라인을 막는다. 첫 수집의 실측값을 보고 PR-1.1에서 승격한다 — UN-1.0 → UN-1.1의 `corpCodeDuplicate`와 같은 패턴이다.
...
`X`를 정하려면 최소한 다음 다섯 분포가 필요하다. 구현이 아니라 이것이 A2b의 첫 과제다.

```
1. A1b 1,222건 중 가격 조회 성공 종목 수
2. 최초 거래일 분포
3. 0행 비율
4. 폐지 연도별 성공률
5. 시장(KOSPI/KOSDAQ)별 성공률
```
