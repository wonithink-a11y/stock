...
> **`failureClassification`은 운영 정책이 아니라 수집 계약의 일부다.**

정책 파일에 있고 이름이 설정값처럼 생겨서 `retryAttempts`·`requestSleepSeconds`와 같은
부류로 읽히기 쉽지만, 성질이 다르다. **재시도 횟수나 간격은 같은 결과에 이르는 경로만
바꾸고, 분류표는 무엇이 수집되고 무엇이 공백이 되는지를 바꾼다** — 어떤 status를 '재시도
불가'로 옮기는 순간 그 법인은 계속 재시도되는 대신 승인 대상 공백이 된다. 그래서 이 두
필드(`nonRetryableStatuses`·`defaultRetryable`)는 수집 계약 해시에 들어가고, 나머지 재시도
파라미터는 들어가지 않는다. 대가는 명시한다: **이 표를 고치면 진행 중인 수집이 폐기된다.**
그래도 '분류 규칙이 바뀌었는데 옛 분류로 모은 상태를 이어받는' 경로를 열어두지 않는 쪽이
옳다 — 그 경로에서 생기는 공백은 조용하다. 표를 고쳐야 할 만큼 새 실패 모드를 발견했다면,
그것은 수집을 다시 시작할 만한 발견이다.
...
표로 검증한다(운영값 10건 → 해시 동일, 계약값 13건 → 해시 변경). 범위를 바꾸려면 그
