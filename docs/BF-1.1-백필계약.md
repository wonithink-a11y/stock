...
### A2b 폐지분 가격 — 구현 완료 (PR-1.4, 2026-08-05), 수집 대기
...
#### 인수 조건 — 구현 완료 (PR-1.4, 2026-08-05). 수집 대기

A2a와 소스·품질 판별(`dailyChange`·`qualityExclusion`)을 공유하고, **네 곳만** 다르다.
다른 곳이 곧 게이트이며 `scripts/test-price-a2b.py`(합성 픽스처 32건)가 넷을 전부 밟는다.

```
1 기대 거래일    실제 최초 거래일 ~ 실제 최종 거래일 (expectedRows.basis = listingPeriod)
                 A2a 기준(캘린더 끝)으로 재면 폐지 이후가 전부 누락이 되어 누락률 90%대
2 빈 응답        정상 결과다. 서킷 브레이커는 예외만 센다
                 정찰 실측 591/1,222(48.4%)가 전 구간 0행이고, A1b가 corp 오름차순이라
                 옛 폐지가 뭉쳐 있어 A2a식 '연속 빈 응답 20건'은 정상 수집을 멈춘다
3 exitAt         마지막 거래일에서만 온다 (delisted-exit.jsonl.gz)
4 커버리지 분모   분석 구간 내 폐지. 후보 전체(51.6%)는 관측으로만 남긴다
```

**FAIL** — 구조적으로 위반이 불가능한 것과 규모 둘

1. `close > 0` · `high >= low` · date 계약 · (ticker,date) 중복 · 캘린더 밖 날짜 ·
   ticker 계약 `[0-9A-Z]{6}` · 품질 제외 후 잔여 ±50% 위반
2. **exitAt 4종** — 소스 값이 `lastTradedDate`인가 · 누락 · 캘린더 거래일인가 ·
   **그 종목이 실제로 거래한 날인가** · 레코드 수 = 확보 종목 수
3. **규모 둘** — `minTickersWithData 600`(정찰 전수 실측 631) ·
   `minTickersInAnalysisWindow 550`(실측 572)

규모 둘을 FAIL로 두는 근거는 **정찰이 표본이 아니라 후보 1,222건 전수**였다는 데 있다.
A3의 정찰 32법인을 게이트로 못 쓰는 것과 여기가 갈리는 지점이다. 여유는 5%·4%로 좁게 잡는다 —
상류를 되풀이하는 임계는 통과율만 떨어뜨리고 새로 잡는 실패 모드가 없다(교훈45).

**exitAt 검사를 '마지막 가격행과 같은가'로 두지 않은 이유**가 같은 교훈이다. 그 비교는
자기 자신을 검사하는 동어반복이라 무엇도 막지 못한다. 그래서 소스를 정책에서 읽어 분기시키고,
`dartModifyDate`로 바꾸는 순간 **'그 종목의 거래일이 아니다'** 에서 걸리게 했다.
막으려는 지름길이 실제로 시도 가능한 경로여야 게이트가 정보를 갖는다.
회귀 테스트가 그 잘못된 경로를 실제로 만들어 FAIL을 확인한다.

**WARN** — 폐지 종목의 실측이 없으므로 여기서 시작한다

`qualityExcludedRateWarn 0.05`(정리매매 구간의 가격 급변이 A2a보다 잦을 수 있다) ·
`missingRateWarn 0.02` · `perTickerMissingRateWarn 0.10` · `emptyRateWarn 0.55` ·
`stillTradingSuspectDaysWarn 5`.

마지막 것은 **A1b 오분류를 잡는 검사**다. A1b는 `A0.7 − A1a` 차집합이라 A1a가 놓친 현재
상장 종목이 폐지 후보로 들어올 수 있다. 마지막 거래일이 캘린더 끝 5거래일 이내면 폐지가
아니라 거래 중일 가능성이 높다. FAIL이 아닌 이유는 폐지 직전 종목이 정상적으로 걸리기
때문이다 — 판정이 아니라 확인 신호다.

**스탬프 둘**(`verify-diagnostics.js`의 `trueFlags`)

```
coverageAssumesFailuresOutOfWindow      확보 실패를 전부 구간 밖으로 본 것은 가정이다
exitAtIsLastTradedNotEffectiveDate      마지막 '거래일'이지 폐지 '효력일'이 아니다
```

둘 다 A2b가 모르는 것이다. 스탬프가 사라지면 하류가 추정을 확정으로 읽는다
(A1b의 `listingHistoryUnverified`와 같은 처리).

산출물은 `data/backfill/price/a2b/{YYYY}.jsonl.gz` · `delisted-exit.jsonl.gz` ·
`price-quality-excluded.jsonl.gz`. **A2a와 디렉터리를 분리한다** — 같은 디렉터리면 manifest
디렉터리 해시가 두 단계를 한 덩어리로 묶어 한쪽 재수집이 다른 쪽 하류까지 재실행시킨다.
샤드 경로(`_shards_a2b`)도 분리한다. 겹치면 A2a finalize가 A2b 샤드를 병합한다.

수집 소요는 **종목당 약 3.4초**(2026-08-05 로컬 실측)로 A2a의 0.3초와 다르다 — 폐지 종목은
상장기간 전체를 한 번에 받고 소스가 요청 구간이 아니라 '오늘로부터 N일'로 동작한다.
1,222건이면 단일 약 70분, 8샤드로 샤드당 약 9분이다.

> **PR-1.4는 A2b를 추가하는 정책 확장이며, A2a 산출물 계약에는 영향이 없어 재생성하지 않았다**
> (2026-08-05). A2a의 입력·처리 로직·acceptance·diagnostics·산출 스키마가 그대로이고,
...
| 경로 | 결과 |
|---|---|
| KRX bld 전체 `getJsonData.cmd` | ❌ **`400 LOGOUT`**. 수급(MDCSTAT02302/02303)만이 아니라 개별종목 일봉(MDCSTAT01701)도 같다. UA·Referer·세션 시드·조회구간 6변형 전부 동일 |
| pykrx `get_market_trading_value_by_date` | ❌ 0행 · 예외 0 (교훈38). 내부가 위 경로다 |
| naver HTML `finance.naver.com/item/frgn.naver` | ✅ 20행/페이지 · 약 2.0초 · **2005-01까지** · `page` 작동 |
| naver JSON `m.stock.naver.com/api/stock/{code}/trend` | ✅ 최대 60행(90은 400) · 약 1.7초 · **`page` 무시 — 최근분만** |

§9.3의 질문 넷에 대한 답:

```
1 종목별 경로가 있는가   KRX ❌ / naver ✅ (두 경로 모두 종목별)
2 기관 세부(연기금·투신) ❌ 두 경로 다 기관 '합계'만 → '기관합계'로 설계한다
3 2016 구간이 오는가      HTML ✅ (2005-01까지) / JSON ❌ (최근 약 60거래일)
4 종목당 소요             HTML 약 130페이지 × 1.8초 ≈ 4분/종목. 3,210종목 ≈ 200시간(단일)
```

부수 실측 넷:

- 폐지종목도 조회된다(`128910`이 2013-06~07을 돌려준다). A2b·A5 연구축과 연결 가능
- 영숫자 티커 `0218L0` 정상(8행). 식별자 계약 위반 없음
- 순매수가 **수량(`...PureBuyQuant`)이지 금액이 아니다.** KR-2.2의 `foreignNetBuy5d`를
  금액으로 정의하려면 소스가 다르다 — 정의를 수량으로 확정할지가 A4 첫 결정이다
- 개인 순매수(`individualPureBuyQuant`)도 함께 온다. KR-2.2에 없는 보조 지표 후보

**이 정찰이 바꾸는 것.** 두 naver 경로는 성질이 다르다 — JSON은 최근분만 싸게 주고,
이력은 HTML이 유일하며 비용이 A3급이다(샤드 resume 필수, Actions 6시간 잡 한도 초과).
그래서 A4는 단일 단계가 아니라 운영(최근분)/이력 두 갈래로 갈릴 후보다. 다만 분할 축을
잘못 고르면 분할의 목적이 사라지므로(교훈46) **결정은 A4 착수 시점에** 한다.

**미확인 1건 — Actions 러너에서의 KRX bld.** 위는 로컬 실측이다. §10 표는 Actions 기준으로
개별종목 일봉을 ✅로 적지만, A2a가 실제로 쓴 경로는 `price.v1.json`의 `adjusted: true`,
즉 **naver**다(`pykrx/stock/stock_api.py`가 `adjusted`면 naver로 간다). 현재 스택에서 KRX bld가
Actions에서 사는지는 확인된 적이 없다. A4 착수 시 1분짜리 probe 잡으로 먼저 가른다.

오늘 하지 않은 것: 정책 파일 작성, 수집기 구현, DART 축(대주주·자사주) 정찰 —
DART 축은 A3와 일 한도를 나눠 쓰므로 A3 수집 중에는 시작하지 않는다.
