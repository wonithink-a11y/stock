...
위 흐름의 '독립 검토'를 누가 하는가는 **그 결과가 무엇을 정하는가**가 정한다.

```
🔴 결정에 닿는 결과      Claude 가 수치를 재현하고 ChatGPT 가 논리·계약·경계를 본다
🔴 에 닿지 않는 결과     ChatGPT 의 논리 검토만으로 충분하다
```

Claude 를 기본 검증자로 두지 않는다. 실험실 산출물의 대부분은 읽기로 검토되며, 실행이
필요한 것은 대용량 재집계뿐이다 — 실측: LAB-5 에서 가장 값진 지적(378건이 조인 실패가
아니라는 것)은 `fundamentals.v1.json` 의 격자 설계를 **읽으면** 나오는 것이었다.

구현과 정책 반영은 이 절이 아니라 기존 승인 규칙을 따른다.
...
실험 산출물의 자리는 `docs/verification/`이다. 비워 두지 않고 첫 산출 때 만들었다
(LAB-5, 2026-08-11) — 빈 디렉터리는 '돌았는데 결과가 없다'로 읽힌다(교훈57).
실험실은 저장소에 쓰지 않으므로 운반은 Claude 가 한다.
